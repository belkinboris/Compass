...
def test_computed_tier_admits_text_facts_but_never_disputed_or_stakeless():
    """Второй сигнал витрины — «рассчитано по тексту карточки» (8 сентября
    2026). Владелец: «мультипликаторов стало слишком мало»; рецензент: три
    сигнала — подтверждено источником / рассчитано «Компасом» / недостаточно
    данных. Второй уровень принимает цену и долю по тексту (basis rule), но
    не спорные факты и не пакет без доли; неизвестный «за что цена» считается
    ценой за пакет."""
    import facts

    base = {
        'id': 'x1', 'type': 'M&A', 'status': 'Закрыта', 'date': '2025-03-01', 'title': 'A купил 60% B',
        'facts': {
            'stake': {'value': 60, 'basis': 'rule'},
            'price': {'value_rub': 1.2e9, 'meaning': 'disclosed', 'basis': 'rule', 'scope': 'unknown'},
            'date': {'basis': 'rule'},
            'nature': {'control_change': True},
            'target': {'company_id': 'b', 'confirmed': True, 'lot': False, 'bank': False},
        },
    }
    ok, why = facts.admitted(base, 'multiple_text')
    assert not ok and why == 'price_not_verified'          # первый сигнал — только после двух чтений
    ok, why = facts.admitted(base, 'multiple_text_computed')
    assert ok, why                                          # второй сигнал — по тексту карточки

    import copy
    no_stake = copy.deepcopy(base); no_stake['facts']['stake'] = {'value': None, 'basis': 'unknown'}
    assert facts.admitted(no_stake, 'multiple_text_computed') == (False, 'stake_unknown')

    whole = copy.deepcopy(no_stake); whole['facts']['price']['scope'] = 'equity'
    assert facts.admitted(whole, 'multiple_text_computed')[0], 'цена за всю компанию не нуждается в доле'

    disputed = copy.deepcopy(base); disputed['facts']['price']['basis'] = 'disputed'
    assert facts.admitted(disputed, 'multiple_text_computed') == (False, 'price_disputed')

    small = copy.deepcopy(base); small['facts']['stake']['value'] = 10
    assert facts.admitted(small, 'multiple_text_computed') == (False, 'stake_below')

    unconfirmed = copy.deepcopy(base); unconfirmed['facts']['target']['confirmed'] = False
    assert facts.admitted(unconfirmed, 'multiple_text_computed') == (False, 'target_unconfirmed')

    # Отвергнутый чтением периметр («Росспиртпром»: головное АО против группы
    # заводов) второй уровень не воскрешает — прочитанное и отвергнутое
    # сильнее непрочитанного текста.
    refuted = copy.deepcopy(base); refuted['facts']['target']['perimeter'] = 'refuted'
    assert facts.admitted(refuted, 'multiple_text_computed') == (False, 'perimeter_refuted')
    split = copy.deepcopy(base); split['facts']['target']['perimeter'] = 'disputed'
    assert facts.admitted(split, 'multiple_text_computed') == (False, 'perimeter_disputed')
