...
def drop_in_process_caches() -> None:
    """Кэшей базы в памяти сайта два: разобранный JSON (`base_data`) и индекс
    ассистента (`assistant_retrieval._INDEX`).

    `base_data` сам замечает подмену файла по времени правки и размеру, так
    что этот вызов ему нужен не для свежести, а для памяти: сбросив ссылку
    сразу, мы не держим старую копию базы (≈72 МБ) до первого запроса.
    Порядок важен — сначала забыть старое, потом греть индекс.
