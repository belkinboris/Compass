...
# Отметка «этот процесс дошёл до обслуживания». Её читает выход по
# остановке (см. `_die_when_no_longer_serving`): гасить процесс, который
# УЖЕ обслуживал и перестал, — лечение зомби; гасить процесс, который ещё
# ни разу не ответил, — превращение неудачного старта в бесконечный цикл
# перезапусков, где платформа не успевает даже написать причину.
_served_at_least_once = False


@app.api_route("/health", methods=["GET", "HEAD"])
def health():
    global _served_at_least_once
    _served_at_least_once = True
...
# ПРОЦЕСС, КОТОРЫЙ БОЛЬШЕ НЕ ОБСЛУЖИВАЕТ ЗАПРОСЫ, ОБЯЗАН УМЕРЕТЬ.
#
# 7 сентября 2026, вечер: сайт отдавал 503 («upstream connect error … remote
# connection failure» от Envoy — прокси НЕ ДОСТУЧАЛСЯ до контейнера) почти
# полчаса, а журнал приложения в это же время выглядел абсолютно здоровым:
# опрос Telegram отвечал 200 каждые 25 секунд, данные подтягивались из GitHub
# каждые 5 минут, ни одного исключения. Разгадка в двух строках выше по
# журналу: в 18:04:45 стояло «Shutting down / Finished server process /
# Application shutdown complete», и после этого НИ ОДНОЙ строки о входящем
# запросе — даже от проверки живости, которая ходит из самого контейнера
# (127.0.0.1). То есть uvicorn перестал слушать порт, а процесс остался жив
# фоновыми потоками и продолжал писать бодрые строки в журнал.
#
# Для платформы это худшее из состояний: контейнер не упал, значит
# перезапускать нечего, — и сайт лежит ровно до тех пор, пока человек не
# нажмёт «пересобрать» руками. Молчаливый зомби опаснее честного падения:
# падение платформа лечит сама за минуту.
#
# Поэтому конец обслуживания = конец процесса. `os._exit` вместо обычного
# выхода намеренно: обычный выход ждёт потоки, а именно потоки здесь и
# держат процесс. Терять при этом нечего — состояние живёт в базе и в git,
# в памяти процесса ничего невосстановимого нет.
@app.on_event("shutdown")
def _die_when_no_longer_serving():
    # В тестах TestClient закрывает приложение штатно на каждом фикстур-выходе —
    # погасив процесс, мы убили бы сам pytest. `PYTEST_CURRENT_TEST` живёт
    # только ВНУТРИ теста и пуст на разборке в конце сессии, поэтому главный
    # признак — сам импортированный pytest.
    import sys
    if ("pytest" in sys.modules or os.environ.get("PYTEST_CURRENT_TEST")
            or os.environ.get("KOMPAS_NO_EXIT_ON_SHUTDOWN")):
        return
    if not _served_at_least_once:
        # Процесс не успел ответить ни на один запрос — значит, это не зомби,
        # а неудачный старт (порт занят, падение в lifespan). Гасить его
        # своими руками нельзя: платформа и так перезапустит контейнер, а мы
        # только спрячем причину под цикл перезапусков.
        logger.warning("uvicorn остановился, не успев обслужить ни одного "
                       "запроса — не гашу процесс, это неудачный старт")
        return
    logger.warning("uvicorn остановился — гашу процесс, чтобы платформа "
                   "перезапустила контейнер, а не считала его живым")
    sys.stdout.flush()
    sys.stderr.flush()
