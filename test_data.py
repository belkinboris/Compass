...
# g1e2b16c3: текст сам себя дословно опровергает как согласование ЭТОЙ
# карточки — «Это согласование относится к более поздней и отдельной
# сделке — продаже всего холдинга, а не к покупке завода S8 Capital в
# 2023 году» (партия FIELD_MISPLACED, 21 сентября 2026: текст перенесён
# из law.appr в eco.context именно потому, что антимонопольное согласие
# было про ДРУГУЮ, более позднюю сделку «Севергрупп»/S8 Capital).
APPROVAL_EXCEPTIONS = {"g4b26b011", "g96e561ef", "g72fad24b", "g718e3d0e", "g5607637d",
                        "gmru-rostech-avia-holding", "g1e2b16c3"}
...
APPROVING_BODY = re.compile(
    ...
    r"|Генпрокуратур[а-яё]*"
    # Австрийская служба госзащиты и разведки (DSN) — она в Австрии отвечает
    # за санкционные разрешения: продажу Sberbank Europe AG австрийскому
    # покупателю одобряла именно она. Тот же класс, что уже принятые OFAC и
    # OFSI, только третья юрисдикция.
    r"|DSN|Дирекц[а-яё]*\s+государственной защиты", re.I)
...
def test_no_company_twins(base):
    ...
    allowed = {frozenset(p) for p in (
        ...
        ("ТД Эдельвейс", "АО «Эдельвейс»"),             # торговый дом и АО — разные юрлица одной группы
    )}
...
def test_match_key_alias_is_a_name(base):
    ...
    Третье — другого рода: «бизнес клаб» это дословная кириллическая
    транслитерация имени профиля «Business Club», а не кусок фразы про
    покупку бизнеса, — просто `flat()` сравнивает строки без транслитерации,
    и латиница с кириллицей не пересекаются подстрокой ни в одну сторону.
    """
    phrase = re.compile(
    ...
    allowed = {"оставшиеся 49 9% акций информ", "долей агроинвест", "бизнес клаб"}
...
# Карточки «Продажа с торгов», у которых продавец ещё не дочитан. Это
# ОЧЕРЕДЬ НА ЧТЕНИЕ, а не разрешение: список только сокращается, и каждая
# снятая строка — прочитанный источник. 13 сентября 2026 (задача #114)
# дочитаны все 26, что стояли в очереди 9 сентября: 25 карточек получили
# продавца дословной цитатой из уже стоявшего в них же текста
# (`pipeline/fix_auction_sellers_batch1.py`), «cf9ca3b8f» оказалась
# дублем, слитым в `gcc677615` (у него продавец — «РЖД» — уже был назван
# раньше). Список пуст — это норма, как и у `NOT_DUPLICATES`: пустой
# список ничего не теряет, он просто значит «непрочитанных не осталось
# на эту дату».
AUCTIONS_WITHOUT_SELLER_YET = set()
