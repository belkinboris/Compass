...
APPROVAL_EXCEPTIONS = {"g4b26b011", "g96e561ef", "g72fad24b", "g718e3d0e", "g5607637d",
                        ...
                        # g5405b876: в «Дополнительной информации» назван приход
                        # ФАС на покупку ВСЕЙ ГК «Агама» импортёром «Юнифрост» —
                        # это соседняя, более поздняя сделка, и текст говорит
                        # это прямо. У сделки самой карточки (выкуп доли Royal
                        # Greenland) согласований не раскрывали. Тот же случай,
                        # что g1e2b16c3: поле пусто честно, а не по недосмотру.
                        "g5405b876",
                        # g925a9d17: «совет директоров… рекомендовал специальный
                        # дивиденд, и акционеры его утвердили» — это решение о
                        # ДИВИДЕНДЕ, а не согласование сделки. Признак теста
                        # (орган + слово об одобрении) на такую фразу срабатывает
                        # по форме; у самой редомициляции согласований источники
                        # не называют, и «—» там честное.
                        "g925a9d17"}
...
APPROVING_BODY = re.compile(
    ...
    # Градостроительная комиссия Москвы согласует условия по инфраструктуре
    # проекта — для сделок с девелоперскими площадками это настоящий орган.
    r"|градостроительн[а-яё]*\s+комисси|градкомисси"
    # Биржа допускает бумагу к торгам, и для IPO это единственное настоящее
    # согласование: у «Инкаб Холдинга» в «Согласованиях» стоит «Стандартные
    # процедуры листинга Мосбиржи». Раньше строка проходила случайно — по
    # слову «акционера» в соседней фразе про локап, которую читатель убрал
    # как условие сделки (ему место в law.terms). Совпадений в базе три, все
    # о настоящем допуске к торгам.
    r"|Мосбирж|Московск[а-яё]*\s+бирж|фондов[а-яё]*\s+бирж"
...
def test_lot_profile_composition_is_named_somewhere_the_reader_sees(base):
    ...
    # Между числительным и существительным стоит прилагательное чаще, чем не
    # стоит: «Две ЛОМБАРДНЫЕ сети», «Две ПРОИЗВОДСТВЕННЫЕ площадки», «Два
    # ПИВОВАРЕННЫХ завода». Без этого пропуска проверка требовала переписать
    # живую русскую фразу под регэксп — то есть ровно наоборот правилу «язык
    # для людей». Слово «фабрик» отсутствовало вовсе, и «Четыре фабрики IKEA»
    # не проходили. Замер 21 сентября 2026: из 57 профилей-лотов фраза стала
    # засчитываться дополнительно у 4, и все четыре называют состав поимённо
    # или числом с регионами.
    lot_phrase = re.compile(
        r"юрлиц|юридическ\w*\s+лиц|одним\s+лотом"
        r"|(?:дв[аеух]|тр[иёе]х?|четыр[еёх]+|пят[ьи])\s+(?:[а-яё]+\s+)?"
        r"(?:предприяти|завод|фабрик|площадк|компани|структур|актив|сет|магазин|отел)", re.I)
...
PARTY_CASE_EXCEPTIONS = {
    ...
    "c2d0c1dcd",                                                      # «Ростех», третья карточка — та же причина, что
                                                                       # у gadb5b474 и ge957fc7b выше
...
def test_an_object_hidden_from_the_catalog_is_never_a_side_of_a_deal(deals, companies):
    """Из каталога «Компании» скрыт объект, а не участник рынка.

    22 сентября 2026 владелец открыл профиль «проект добычи золота
    Быстринско-Ширинское» и сказал убрать: в компаниях остаются компании, а
    не дома и проекты. Удалять такие профили нельзя — 131 из них стоит
    ПРЕДМЕТОМ реальных карточек, и удаление увело бы плашку сторон в никуда.
    Поэтому профиль живёт, а из каталога уходит по признаку `asset`.

    Опасная сторона ошибки здесь одна: спрятать того, кто на самом деле
    покупал или продавал. Имя обманывает — профиль «Ingka Centres (портфель
    ТЦ «Мега»)» назван портфелем, а выступает ПРОДАВЦОМ в трёх сделках.
    Признак роли надёжнее имени, и тест держит именно его.
    """
    hidden = {cid for cid, c in companies.items() if c.get("asset")}
    assert hidden, "признак каталога пропал из базы"
    bad = []
    for d in deals:
