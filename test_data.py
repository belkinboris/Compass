...
# g5607637d: согласование ФАС в тексте относится к НЕСОСТОЯВШЕЙСЯ сделке
# другого покупателя («Юг Руси»), а не к той, что реально закрылась
# («Сигнет Инвестиции») — переносить его в `law.appr` действующей карточки
# значило бы приписать согласование не той сделке (заметка владельца 480,
# консоль, 4 сентября 2026).
# gmru-rostech-avia-holding: «совет директоров... не утвердил заместителей
# гендиректора» — это провал ВНУТРЕННЕГО кадрового решения Red Wings
# (конфликт менеджмента), а не одобрение самой сделки консолидации каким-
# либо органом; `law.appr` честно пуст, потому что согласования сделки
# источники не называют вовсе (6 сентября 2026).
APPROVAL_EXCEPTIONS = {"g4b26b011", "g96e561ef", "g72fad24b", "g718e3d0e", "g5607637d",
                        "gmru-rostech-avia-holding"}
...
APPROVING_BODY = re.compile(
    ...
    r"|совет[а-яё]*\s+директоров|собрани[а-яё]*\s+акционеров|акционер|указ|распоряжени"
    r"|предписани"
    # `суд\b` не совпадал со словом в косвенном падеже («суде», «суда») —
    # после корня сразу шла буква окончания, а не граница слова. У «Самолёт»/
    # «Лидер строй» единственное упоминание согласования — «Газпромбанк
    # согласовал схему... «Лидер строй» оспорила в арбитражном суде решение
    # общего собрания» — и правило молчало ровно из-за формы слова. Родня
    # уже записанного класса ошибок с `\b` после приставки/окончания:
    # граница слова нужна ПЕРЕД корнем, а не после него.
    r"|\bсуд[а-яё]*"
    # Прокуратура районного/областного уровня — тот же класс, что уже
    # принятая «Генпрокуратур[а-яё]*», только без федеральной приставки: у
    # того же «Самолёт»/«Лидер строй» прокуратура Первореченского района
    # Владивостока вынесла застройщику предостережение о нарушении закона —
    # официальное действие госоргана, просто не Генпрокуратуры РФ.
    r"|прокуратур[а-яё]*"
    ...
    r"|ЦБ\b"
    # «Руководство страны» без уточнения — тот же класс, что «органы
    # власти»/«регулирующие органы» выше: у Башкирии/«Роснефти» премьер
    # республики Андрей Назаров подтвердил согласование именно этой формулой
    # («реализовали с согласия руководства страны»), не называя орган
    # поимённо.
    r"|руководств[а-яё]*\s+страны"
    ...
    r"|OFSI"
    # Генпрокуратура — согласующий/инициирующий орган для национализации
    # через суд (не банкротство по 127-ФЗ): у Домодедово решение о взыскании
    # активов в госсобственность приняли суды по иску именно этого органа
    # («первая инстанция удовлетворила требование Генпрокуратуры»), а слова
    # «суд» в тексте нет — есть «инстанция», «апелляция», «кассация». Родня
    # уже принятого Роскосмоса: источник называет орган по имени, но не тем
    # словом, которое уже есть в списке.
    r"|Генпрокуратур[а-яё]*", re.I)
...
def test_lot_profile_composition_is_named_somewhere_the_reader_sees(base):
    """У профиля-лота состав назван — в имени или в описании.

    Признак `lot` ставится записи, за которой в одной сделке шли несколько
    юридических лиц, и интерфейс говорит об этом отдельной строкой («Это не
    одна компания…»). Раньше тест требовал, чтобы юрлица были перечислены в
    ИМЕНИ, — и это прямо противоречило правилу «имя профиля — компания, а не
    состав сделки» (просьба владельца 3 сентября 2026,
    `pipeline/fix_company_names_are_real_companies.py`): у лота с общим
    брендом имя теперь «Термоклип», а перечень юрлиц переехал в описание.

    Инвариант не отменён, а заменён на более слабый и всё ещё механический:
    состав обязан быть НАЗВАН где-то, где читатель его видит. Либо имя
    перечисляет («ООО «Датана» и ООО «Датабриз»» — так остались лоты без
    общего бренда), либо описание называет не меньше двух юрлиц или прямо
    говорит, сколько их и что они шли одним лотом.
    """
    enumerating_name = re.compile(r"\bи\b|,|юрлиц|компании,", re.I)
    entity = re.compile(r"(?<![А-Яа-яЁё])(ООО|АО|ЗАО|ПАО|ОАО|СХП|ГК|ИК)(?![А-Яа-яЁё])")
    lot_phrase = re.compile(
        r"юрлиц|юридическ\w*\s+лиц|одним\s+лотом"
        r"|(?:дв[аеух]|тр[иёе]х?|четыр[еёх]+|пят[ьи])\s+"
        r"(?:предприяти|завод|площадк|компани|структур|актив|сет)", re.I)
    ...
    for cid, c in base["companies"].items():
        ...
        if enumerating_name.search(name):
            continue
        if len(entity.findall(desc)) >= 2 or lot_phrase.search(desc):
            continue
        bad.append((cid, name))
    assert not bad, f"лот не называет свой состав ни в имени, ни в описании: {bad[:5]}"
...
def test_artem_feedback_ui_invariants():
    ...
    html = INDEX.read_text(encoding="utf-8")
    # `.hero-brand` — отдельный логотип «КОМПАС» ВНУТРИ героя — был добавлен
    # взамен пропавшего логотипа, но `.top .wordmark` (фиксированная шапка,
    # z-index:50) и так лежит поверх героя ВСЕГДА, включая исходное
    # положение прокрутки: 16 августа 2026 владелец заметил, что название
    # видно ДВАЖДY на одном экране — на плашке и на самой картинке героя.
    # Тот же класс, что «Экономист» и «Юрист», дублирующие «Цель сделки»:
    # правило заменено на противоположное, а не отменено без замены.
    assert 'class="hero-brand"' not in html, "дубль логотипа в герое вернулся"
    assert 'class="wordmark"' in html, "логотип пропал из шапки совсем"
    assert 'window.hsTimer=null' in html and 'hsTimer=setInterval' not in html, \
        "автопереключение слайдера вернулось"
    # 18 августа 2026: карточка и так короткая, честное «ничего не найдено»
    # всё равно читается как обещанный, но пустой блок — теперь при пустой
    # подборке «Похожие сделки» не рендерятся вовсе (relatedHtml возвращает
    # ""), а не показывают текст-объяснение. Обе прежние формулировки этого
    # текста ПРОВЕРЯЕМ НА ОТСУТСТВИЕ — если он вернулся, значит откатили
    # правку, не заметив её причины.
    assert 'Сделок с теми же сторонами или общей темой пока не найдено' not in html
...
# Карточки «Продажа с торгов», у которых продавец ещё не дочитан (замер
# 9 сентября 2026 — 26 из 47). Это ОЧЕРЕДЬ НА ЧТЕНИЕ, а не разрешение:
# список только сокращается, и каждая снятая строка — прочитанный источник.
AUCTIONS_WITHOUT_SELLER_YET = {
    'caac79625',  # Продажа Росэлектроникой 100% акций АО «Спецмагнит» с недвижимостью
    'ce7b84bec',  # Сбербанк Капитал и КР Плюс выставили на аукцион 100% долей в «Юг Д
    'cf9ca3b8f',  # РЖД продает Рижский вокзал в Москве
    'domodedovo-aukcion',  # Шереметьево приобрёл активы Домодедово на аукционе на понижение
    'g016780a4',  # Продажа имущественного комплекса шахты «Анжерская-Южная» (ООО «ОЭУ
    'g01e3fd26',  # Росимущество выставило на торги бывшие активы производителя водки 
    'g06d3e232',  # Продажа бизнес-центра «Чайка-плаза 10» на торгах в рамках банкротс
    'g171f9461',  # Группа ВТБ приобрела отель Courtyard by Marriott Kazan Kremlin на 
    'g1dc82018',  # Приобретение ООО «СК С.Т.С.» (ГК «Садовое кольцо») Серпуховского л
    'g259866c4',  # Выставление на продажу завода «Камский сапропель» за 350 млн руб. 
    'g323cf076',  # АО «УК «Белая скала»» приобрела складской портфель Raven Russia (1
    'g4ebb84f2',  # Покупка имущественного комплекса Волховского НПЗ (ООО «Волховнефте
    'g53ebfe72',  # Продажа имущественного комплекса ООО «Донстар» (производитель мяса
    'g5f662edc',  # Приватизация 100% акций АО «Международный Аэропорт Магнитогорск» —
    'g64837e3a',  # Продажа бизнес-центра «Сириус» (ООО «Крепость-Сириус») в Красноярс
    'g7527482e',  # Реализация производственно-технологического комплекса ООО «Евродон
    'g7eb917c0',  # Публичные торги по реализации имущественного комплекса ОАО «Санато
    'g7f02f562',  # Продажа торгово-логистического центра «Юлмарта» на Пулковском шосс
    'g9c8c34b4',  # Выставление на торги активов ООО «Виват-Трейд» (Пермский край) еди
    'ga0427eac',  # Продажа акций АО «Разрез «Степановский»» (Новокузнецк) на торгах в
    'gb764d1a9',  # Выставление на торги ТРЦ «Акварель» (Тамбов) в рамках банкротства 
    'gcc232775',  # Роснефть выкупила Саянскхимпласт на торгах за 30,3 млрд рублей
    'gd01de1ad',  # Признание недействительной сделки по продаже магазинов Podium Mark
    'gdb820839',  # Продажа сайта Mediazavod.ru обанкротившегося медиахолдинга ЗАО «ЧР
    'gec8d0f1a',  # АО «Интер Карго Компани» приобрела производственный комплекс АО «Э
    'gmru-rodnye-polya',  # Бизнес «Родных полей» продан за 11,7 млрд рублей
}


def test_auction_deal_names_its_seller(deals):
    """У продажи с торгов продавец назван — это публичная процедура.

    Владелец 9 сентября 2026 о карточке «Амбер Талвиса»: «если ты в этой
    карте ещё и продавца не можешь идентифицировать, то я буду вообще
    разочарован (это следует практически из всех источников и прямо указано
    в источнике про сам аукцион)». Он прав по существу, и это не про одну
    карточку: на торгах у лота ВСЕГДА есть названный продавец — Росимущество,
    конкурсный управляющий, банк-залогодержатель, госкомпания. «Продавец не
    раскрыт» здесь не честная пустота (как у частной сделки, где стороны
    договорились молчать), а непрочитанный источник.

    Поэтому список известных непрочитанных зафиксирован поимённо: новая
    карточка с торгов без продавца роняет тест сразу, а старые снимаются по
    мере дочитывания. Тот же приём, что у `NOT_DUPLICATES` в сканере дублей:
    исключение существует, но названо и конечно.
    """
    import re
    placeholder = re.compile(
        r"^\s*(—|-|не раскрыт[а-яё]*|публично не сообщалось|нет данных)?\s*\.?\s*$", re.I)
    bad = [d["id"] for d in deals
           if d.get("type") == "Продажа с торгов"
           and not d.get("seller_id")
           and (not d.get("seller") or placeholder.match(str(d["seller"])))
           and d["id"] not in AUCTIONS_WITHOUT_SELLER_YET]
    assert not bad, (
        "у сделки с торгов не назван продавец (на торгах он всегда назван): %s" % bad[:5])


def test_source_label_matches_the_link(deals):
    """Подпись источника называет то, куда ведёт ссылка.

    Владелец 9 сентября 2026: «нажимаю на источник в этой новости, а перехожу
    не на телеграм канал, который там показан @DEALSMA (как так вообще?), а
    на torgi.gov». Класс записан в CLAUDE.md с августа («Источник — то, что
    подтверждает факт, а не то, как о нём узнали»), кампания тогда
    переподписала 913 карточек — и хвост в 89 карточек нарос заново, потому
    что чинили накопленное, а забора не поставили. Забор: подпись, обещающая
    Telegram, допустима только на ссылке в Telegram.
    """
    from pipeline.source_names import host_of, label_promises_telegram
    bad = []
    for d in deals:
        for item in (d.get("src") or []):
            ...
            if not url.startswith("http"):
                continue
            host = host_of(url)
            if label_promises_telegram(label) and not host.endswith(("t.me", "telegram.me")):
...
# Денай/опровержение остаётся в law.struct/law.terms сознательно — сюжет
# самой карточки И ЕСТЬ денай, поверх которого другого «устройства сделки»
# нет, и это решение зафиксировано отдельным разовым скриптом с полным
# разбором хронологии, а не случайный автоимпорт.
DENIAL_ALLOWED_IN_LAW = {
    "g79f683bc",  # Фонбет/Stone Курская — карточка целиком про денай,
                  # статус «Не состоялась», разбор pipeline/
                  # fix_fonbet_stone_kurskaya_denial.py (21.08.2026)
    "gfcfba8c9",  # Renaissance/«Аура» — денай касается только сургутской
                  # половины лота, статус осознанно не менялся, разбор
                  # pipeline/fix_aura_surgut_denial.py (30.08.2026)
}


def test_law_fields_do_not_carry_denials_or_stale_speculation(deals):
    """«Как устроена сделка» и «Условия сделки» — не место для опровержения
    или неснятой гипотезы «возможно».

    Владелец 11 сентября 2026, отвечая на разосланные вопросы, нашёл три
    карточки подряд с одним и тем же классом дефекта: `review.py` проверяет,
    что цитата реальна и точно приведена, но не проверяет, что она подходит
    ПО СМЫСЛУ полю, в которое записана (тот же класс, что уже описан в
    CLAUDE.md для HeadHunter/Happy Job, август). У БКС/«Форштадт» и S8
    Capital/«Аквариус» опровержение или устаревшая гипотеза о механизме
    сделки («возможно, будет проведена через SPV») лежали в law.struct —
    поле для структуры сделки, а не для денай-цитаты, которая к тому же
    почти всегда уже полностью рассказана в eco.context. У «Мать и дитя»/
    «Инвитро» в law.terms («Условия сделки», линза «Юрист») лежал тот же
    класс — не условие, а более ранняя версия денай-факта.

    Вычитка (`proofread.py`) это не ловит: она проверяет только стиль,
    не адрес поля — отпечаток в `proofread_absorbed` показывает, что она
    прошла ровно по этим полям и не заметила проблему, потому что это не
    её вопрос. Дословность (`review.py`) тоже не ловит: цитаты настоящие.
    Единственная проверка, которая может поймать класс «текст не по
    смыслу поля», — прямая, по словам денай/спекуляции.

    Два известных случая, где денай сознательно ОСТАВЛЕН в law.struct
    (не автоимпорт, а разбор с полной хронологией), — исключены поимённо
    в `DENIAL_ALLOWED_IN_LAW`, тем же приёмом, что `AUCTIONS_WITHOUT_
    SELLER_YET` и `NOT_DUPLICATES`.
    """
    import re

    denial_re = re.compile(r"не соответствует действительности|опроверг|опровержени", re.I)
    speculative_re = re.compile(r"возможно,?\s*будет\s+провед|может быть проведена через", re.I)

    bad = []
    for d in deals:
        if d["id"] in DENIAL_ALLOWED_IN_LAW:
            continue
        law = d.get("law") or {}
        for field in ("struct", "terms"):
            text = law.get(field)
            if not isinstance(text, str):
                continue
            if denial_re.search(text) or speculative_re.search(text):
                bad.append((d["id"], field, text[:100]))
    assert not bad, (
