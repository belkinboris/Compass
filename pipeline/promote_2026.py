...
INDUSTRIES = {
    "Нефть и газ", "Уголь", "ГМК и добыча", "Энергетика", "Химия и удобрения", "Агро",
    "Пищепром и напитки", "Ритейл", "E-commerce", "Потребительские товары", "ИТ и интернет",
    "Искусственный интеллект", "Телеком", "Банки", "Страхование", "Финтех",
    "Финансовые услуги", "Рынок ценных бумаг", "Управление активами", "Холдинги",
    "Транспорт и логистика", "Порты и инфраструктура", "Автопром", "Недвижимость",
    "Строительство", "Гостиницы и туризм", "Здравоохранение", "Фармацевтика",
    "Образование", "Развлечения", "Профессиональные услуги", "Медиа", "Машиностроение",
    "Лесопром", "ЖКХ и обращение с отходами", "Не определена",
}
...
def resolve_company(name, new_companies, match_keys, ind, review):
    ...
    # Раньше требовали len>3 — из-за этого короткие акронимы вроде "ВТБ",
    # "РЖД", "VK" оставались вообще без ключа поиска и были ненаходимы в
    # bulk/mini/channel-упоминаниях. containsKey() на фронте уже проверяет
    # границы слова для ключей длиной ≤4 — этого достаточно, чтобы не ловить
    # случайные подстроки внутри других слов.
    match_keys[cid] = [norm] if len(norm) > 1 else []
    return cid, True
...
def main():
    ...
    for r in good:
        ...
        ind = ind_raw if ind_raw in INDUSTRIES else IND_FALLBACK.get(ind_raw, ind_raw or "Не определена")
