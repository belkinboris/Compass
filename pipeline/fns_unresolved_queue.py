...
def attempt_single_exact_match(client, name):
    ...
    Композиция из двух отдельных функций (`attempt_public_egrul_match` +
    `attempt_paid_exact_match`), а не единый метод, — нарочно: `main()`
    зовёт бесплатный шаг для КАЖДОГО кандидата напрямую, а платный —
    только когда бесплатный не нашёл и обрыватель ещё не сработал (см.
    докстринг `attempt_paid_exact_match`, почему это разделение важно)."""
    hit = attempt_public_egrul_match(name)
    if hit:
        return hit
    return attempt_paid_exact_match(client, name)


def attempt_paid_exact_match(client, name):
    """Вторая, платная половина `attempt_single_exact_match` — тот же
    критерий (единственное действующее точное совпадение), но только
    через `search` api-fns.ru. Вынесена в отдельную функцию 19 сентября
    2026: `main()` раньше вызывал бесплатный и платный шаг ОДНИМ вызовом
    `attempt_single_exact_match`, и обрыватель на 3 подряд ошибках `search`
    останавливал вообще весь цикл — то есть оставшиеся кандидаты той же
    очереди не получали даже БЕСПЛАТНОЙ попытки, хотя она с платным API
    не связана и её отказ никак не предсказывает. На живом прогоне того
    дня платный `search` отвечал 403 на первые же три компании — и
    единственный обрыватель молча списал все 97 кандидатов очереди как
    «нужен ИНН вручную», хотя бесплатный ЕГРЮЛ в одиночку в тот же день
    подтвердил 36 из них. Теперь `main()` вызывает эту функцию отдельно от
    бесплатной, и обрыватель останавливает только ПЛАТНЫЕ попытки — цикл
    бесплатного поиска для всех кандидатов очереди идёт независимо."""
...
def main():
    ...
    if not args.attempt:
        ...
        # Этап 13, П4: ошибка ПЛАТНОГО API (сеть, ключ, исчерпанная ГОДОВАЯ
        # квота `search` — 3000/год на метод, отдельно от bo/egr/changes)
        # не должна тихо превращаться в «честно не нашли». Три подряд
        # ошибки — признак систематического отказа платного шага (одна
        # компания без совпадения не значит, что API вообще недоступен), и
        # платные попытки после этого прекращаются.
        #
        # 19 сентября 2026: раньше обрыватель останавливал ВЕСЬ цикл, в том
        # числе бесплатный ЕГРЮЛ-поиск для кандидатов, до которых очередь
        # ещё не дошла, хотя его отказ платным API никак не предсказан (см.
        # докстринг `attempt_paid_exact_match`). Теперь бесплатный шаг —
        # для КАЖДОГО кандидата, платный — только пока обрыватель не
        # сработал; после срабатывания цикл идёт дальше по всем кандидатам
        # бесплатным поиском в одиночку, а не останавливается совсем.
        confirmed, queue, api_errors = [], [], 0
        paid_disabled = False
        with ApiFnsClient() as client:
            for i, (cid, name, date, count) in enumerate(rows):
                hit = attempt_public_egrul_match(name)
                if not hit and not paid_disabled:
                    try:
                        hit = attempt_paid_exact_match(client, name)
                    except ApiFnsError as exc:
                        api_errors += 1
                        if api_errors >= 3:
                            paid_disabled = True
                            print("\n⛔ Платный поиск ФНС недоступен (%d ошибок подряд/из %d "
                                  "попыток, последняя: %s) — похоже на исчерпанную квоту или "
                                  "сбой сети, не на честное «не нашли». Платные попытки "
                                  "отключены до конца прогона, бесплатный ЕГРЮЛ-поиск "
                                  "продолжается для оставшихся %d кандидатов."
        ...
        if api_errors:
            print("Ошибок платного API при поиске (не в счёт «не нашли»): %d из %d попыток."
                  % (api_errors, len(rows)))
