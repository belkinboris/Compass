...
def candidates(deals):
    ...
    # ПЕРЕГОВОРЫ, КОТОРЫЕ ТАК И НЕ ЗАКРЫЛИСЬ, — ЕСЛИ НА ТОМ ЖЕ ПРЕДМЕТЕ ПОЗЖЕ
    # РЕАЛЬНО ЗАКРЫЛАСЬ СДЕЛКА. Владелец сам нашёл 13 сентября 2026: карточка
    # «М Холдинг Лтд приобретает ТЦ «Метрополис» у Morgan Stanley и Hines»
    # (2022, статус «Обсуждается» навсегда — переговоры сорвались) и карточка
    # «Продажа ТРЦ «Метрополис» фонду Balchug Capital» (апрель 2023, реальное
    # закрытие) — это ОДНА сделка, рассказанная сначала со стадии, которая не
    # состоялась, а не два разных сюжета. Правила выше её не находили: общий
    # ПРОФИЛЬ предмета у обеих карточек есть, но год разный (2022 vs 2023) и
    # сумма разная (обе не раскрыты), а «один предмет» выше требует год ИЛИ
    # сумму. У сорвавшихся переговоров и реального закрытия почти никогда нет
    # ни того, ни другого — это и есть признак: статус «Обсуждается» без
    # обновления рядом с профилем, на котором ПОЗЖЕ (в любом году) стоит любая
    # другая сделка. Порог по времени НЕ нужен: если статус год не менялся,
    # разница в 3 месяца и в 3 года читаются одинаково. Список для чтения, не
    # приговор — замер 13 сентября 2026 (до чтения) дал 29 пар на всей базе, из
    # них минимум две трети похожи на обычные повторные сделки с одной и той же
    # компанией (IPO/SPO/новый раунд), не на тот же сюжет дважды; читать и
    # записывать в NOT_DUPLICATES по мере разбора, как и остальные признаки
    # этого файла.
    for group in by_asset.values():
        discussing = [d for d in group if d.get('status') == 'Обсуждается']
        others = [d for d in group if d.get('status') != 'Обсуждается']
        for a in discussing:
            for b in others:
                found.setdefault(frozenset({a['id'], b['id']}),
                                 'переговоры без исхода на том же предмете: '
                                 '«%s» (%s) рядом с «%s» (%s)'
                                 % (a.get('status'), a.get('date'), b.get('status'), b.get('date')))
    return found
