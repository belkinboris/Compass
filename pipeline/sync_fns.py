...
# Формы собственности, которые снимаются с имени перед сравнением. Составные
# формы стоят ПЕРЕД простыми и длинные — перед короткими: «публичное
# акционерное общество» обязано сняться целиком, иначе от него останется
# «публичное», и ПАО «Группа Позитив» никогда не совпадёт с ЕГРЮЛ (найдено
# 11 сентября 2026: бесплатный поиск ЕГРЮЛ вернул за ПАО «Группа Позитив»
# ликвидированное красноярское ООО с тем же именем — единственное «точное»
# совпадение, потому что у настоящего ПАО форма не снялась). «Международная
# компания» — приставка резидентов САР (МКАО/МКООО), у «Совко Капитал
# Партнерс» она стоит в официальном имени.
_OPF = re.compile(
    r"\b(международная компания|публичное акционерное общество|непубличное акционерное общество|"
    r"акционерное общество|общество с ограниченной ответственностью|"
    r"автономная некоммерческая организация|"
    r"ооо|ао|пао|нао|оао|зао|мкао|мкооо|нко|банк|фгбу|фгаоу|фгуп|муп|гуп)\b", re.I
)
...
def sync_from_registry(db, client: ApiFnsClient, *, limit: int | None = None,
    ...
    confirmed = [row for row in FNS_REGISTRY if registry_row_is_syncable(row)]
    # Новые строки реестра — первыми. 5 сентября 2026 подтверждённый ИНН
    # «МорТехПрома» (карточка из свежего поста канала) стоял ПОСЛЕДНИМ в
    # файле, а перед ним — 278 строк кампании по сторонам сделок 2024 года,
    # ещё не доехавших до прода: при 60 работах на старт и дневном потолке
    # (два старта в сутки) читатель, пришедший из поста сегодня, ждал бы
    # отчётность несколько дней. Приток добавляет по строке в день, кампании —
    # партиями по 50-60 в тот же день, поэтому порядок по дате строки (новые
    # раньше) отдаёт свежие сделки первыми, а хвост кампании догоняет
    # следующими стартами. Сортировка устойчивая: внутри одной даты —
    # порядок файла.
    confirmed.sort(key=lambda row: str(row.get("date") or ""), reverse=True)
    cutoff = _now() - timedelta(days=REGISTRY_SYNC_STALE_DAYS)
    work_done = 0
    for row in confirmed:
