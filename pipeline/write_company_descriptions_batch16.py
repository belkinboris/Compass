# -*- coding: utf-8 -*-
"""Шестнадцатая партия описаний профилей: чем компания занимается.

ЧТО СЛОМАНО. После пятнадцати партий настоящее описание есть у 551
профиля из 1872 (29%). Кандидатов этой партии искали по узнаваемости
имени, а не по рангу участия в сделках (см. батчи 8–15) — каждый проверен
по тексту его собственной сделки в базе. Узнаваемые имена без единой
сделки в базе отложены намеренно — Роквул (российские активы), ПЭК:
...
Отдельная находка при чтении контекста: «Тетра» стояла в «Пищепром и
напитки» — собственная сделка называет её прямо «производителем и
дистрибьютором туалетной бумаги», а не пищевой компанией. Исправлено на
«Потребительские товары» — тот же класс дефекта, что у ООО «Эссити» в
батче 15 (гигиенические бумажные изделия ошибочно попадают в пищепром).

Запуск:
    python3 pipeline/write_company_descriptions_batch16.py            # сухой прогон
    python3 pipeline/write_company_descriptions_batch16.py --write    # записать
"""
import json
import re
import sys
...
DESCRIPTIONS = {
    'g24e20170': 'Российский производитель туалетной бумаги и бумажных '
                 'изделий.',
    'g83b10f66': 'Российский банк для автокредитования, ранее совместное '
                 'предприятие Renault-Nissan и UniCredit.',
    'g18cd31cf': 'Российская сеть медицинских лабораторий.',
    'g73cfb717': 'Российская птицефабрика, входила в агрохолдинг «Продо».',
    'ga56731fe': 'Российская инфраструктурная и угледобывающая группа.',
}
...
INDUSTRY_FIXES = {
    'g24e20170': ('Пищепром и напитки', 'Потребительские товары'),
}
