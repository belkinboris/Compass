...
Кроме адресов из самих карточек скрипт забирает и адрес, ПРЯМО НАЗВАННЫЙ в
аргументах, — это второй источник, найденный поиском для карточки, у которой
первый исчерпан (см. докстроку `main`). Доверия это адресу не прибавляет:
чтобы факт из него попал в карточку, `review.py` потребует и дословную цитату,
и присутствие адреса в кэше.
...
Запуск:
    ...
    python3 pipeline/ingest/fetch_article_texts.py https://... --write  # второй источник
"""
...
def main(args, write=False):
    """Аргументы — id карточек и/или прямые адреса статей.

    Прямой адрес — это ВТОРОЙ ИСТОЧНИК, найденный поиском для карточки, у
    которой первый источник исчерпан. Замер 10 августа: у 80% прочитанных
    карточек источник ровно один, и после честного чтения линза «Юрист»
    остаётся пустой у 56%, а обе линзы бедны у 29% — потолок здесь не в
    чтении, а в том, что одна заметка про сделку не пишет ни о заверениях,
    ни о согласованиях.

    Скачать статью — не то же самое, что поверить ей. Дальше действует
    обычный порядок: адрес дописывается в `src` карточки через `review.py`,
    который проверяет, что этот адрес ДЕЙСТВИТЕЛЬНО забран и лежит в кэше, а
    каждая цитата дословно лежит в скачанном тексте. Поэтому найденное
    поиском попадает в карточку ровно теми же воротами, что и всё остальное,
    и «правдоподобный пересказ из выдачи» через них не проходит.
    """
    ids = [a for a in args if not a.startswith('http')]
    found = [a for a in args if a.startswith('http')]
    targets = card_urls(ids) + [('найден поиском', u, '') for u in found]
    fetched, failed = fetch_and_store(targets, write=write)
    if not write:
