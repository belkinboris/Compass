...
def proposals(deal, item, names, comps, match_keys=None):
    ...
    if moved and informative_title_update(deal.get('title'), item.get('title')):
        # normalize_quotes() — иначе тот же баг четвёртый раз: заголовок
        # источника набран прямыми кавычками, а `draft.build()` для НОВЫХ
        # карточек их уже красит в « », это же нужно и здесь, при смене
        # стадии на СУЩЕСТВУЮЩЕЙ карточке (найдено на g70c0a9ff, 3 сентября
        # 2026 — «НМГ купила контрольный пакет ИД "Комсомольская правда"»
        # ушло в базу с прямыми кавычками).
        out.append(('title', draft.normalize_quotes(str(item.get('title')).strip()),
                    'обновить', 'заголовок обновлён по источнику новой стадии'))
