...
# Обрыв текста многоточием на середине предложения — НЕ регрессия (проверено
# 13 сентября: `truncate_note()` в draft.py — единственное место записи
# `events[].note` во всём пайплайне, и работает верно: режет по границе слова
# и честно ставит «…», а не молча посередине слова). Девять найденных живых
# случаев старше самого гейта приёмки или старше этой проверки — чего не
# хватало, так это узла, который ловит НЕЗАКРЫТЫЙ обрыв ПЕРЕД публикацией, а
# не переписывает код, который его производит. Узкий признак (текст
...
SINGLE_LEGAL = re.compile(
    r'^\s*(?:ООО|ОАО|ПАО|ЗАО|НАО|АО|МКПАО|МКООО|АНО|ГУП|ФГУП|МУП|НПАО)\s*«.+»\s*$', re.I)
# Закрывающая ёлочка, за которой идёт запятая или «и», — это перечисление,
# а не одно имя: «АО «Норд-Вест СВ», «Инструмент-СВ»…». Внутри ОДНОГО имени
# кавычки бывают вложенными («ООО «Кинокомпания «Парадиз продакшнз»»»),
# поэтому берём жадно до последней ёлочки и отсекаем списки отдельно.
LEGAL_LIST = re.compile(r'»\s*(?:,|и\s)')


def single_legal_entity(text):
    """Имя, если текст роли называет РОВНО ОДНО юрлицо и больше ничего.

    Зачем отдельное правило, а не просто «встретилось ООО». Владелец
    23 сентября 2026: «мы же на этапе черновика уже можем найти профиль и
    посмотреть отчётность». Может — но только когда названо одно юрлицо.
    Замер по базе в тот же день: из 29 отложенных профилей юрлицо названо у
    четырёх, и у двух из них откладывать ПРАВИЛЬНО — «АО «Норд-Вест СВ»,
    «Инструмент-СВ», «Машиностроение СВ»…» (лот из пяти) и «ООО «Некстеп»,
    Теймур Штернлиб и Дмитрий Васильев» (юрлицо вместе с физлицами). Для них
    профиль одной кнопкой не заводится. А «ООО «Розница К-1»» — заводится, и
    вместе с ним приходят ИНН из ЕГРЮЛ и отчётность из ГИР БО.
    """
    t = ' '.join(str(text or '').split())
    if LEGAL_LIST.search(t):
        return None
    return t if SINGLE_LEGAL.match(t) else None
...
FIN_IS_PAYMENT = re.compile(
    r'(денежн|деньгами|наличн|рассрочк|взаимозач|аккредитив|эскроу|заемн|заёмн|'
    r'оплат|расч[ёе]т|транш|рублями|обмен|финансир|кредит|долг|выплат|'
    r'структура цены|погашен|вкладов|инвестиру)', re.I)
FIN_IS_REPORTING = re.compile(
    r'^(По данным (СПАРК|отч[ёе]тности|ЕГРЮЛ|РСБУ|МСФО)|Выручка|Чистая прибыль|'
...
def findings(card, base, waived=None, waived_inn=None, waived_sources=None, registry=None,
    ...
    # Аудит 13 сентября 2026, сквозной паттерн №1: поле «Форма расчётов»
    # (eco.fin) систематически несло выручку и прибыль ПОКУПАТЕЛЯ вместо
    ...
    if fin_text and FIN_IS_REPORTING.match(fin_text) and not FIN_IS_PAYMENT.search(fin_text):
        out.append(('fin_is_reporting',
                    ...
                    'а не то, чем платили. Показатели покупателя — в «Историю и '
                    'окружение», покупаемой компании — в «Финансы покупаемой компании»'
                    % fin_text[:70]))
    ...
    for key in ('no_profile', 'no_inn', 'name_ok'):
        for role, reason in (ans.get(key) or {}).items():
            if role not in ROLES:
                ...
                # «Профиля ещё нет» — не причина, а задача: имя юрлица уже
                # названо в карточке. Отложить можно лот, список сторон или
                # физлицо; одно юрлицо — заводится, и вместе с профилем
                # приходят ИНН и отчётность. Без этого пост о сделке выходит
                # без финансов предмета, хотя они были в двух кликах
                # («Лента»/«Мария-Ра», 23 сентября 2026).
                text = str(card.get(ROLES[role][0]) or '')
                named = single_legal_entity(text)
                if named:
                    bad.append('no_profile: %s назван одним юрлицом «%s» — заведите профиль, '
                               'а не откладывайте: имя есть, ИНН ищется по ЕГРЮЛ, и тогда '
                               'отчётность подтянется сама' % (role, named))
                # «Профиля в базе ещё нет» — утверждение, и его никто не
                # проверял. У «Марии-Ра» профиль существовал с 2023 года
                # (карточка ТРЦ «Саларис», где сеть была покупателем), а
                # читатель отложил роль именно с этой причиной.
                key = link_parties.company_key(re.sub(
                    r'^(?:ООО|ОАО|ПАО|ЗАО|НАО|АО|МКПАО|МКООО)\s*', '', text).strip('«» "'))
                twin = [c for c, v in comps.items()
                        if len(key) >= 3 and link_parties.company_key(v.get('name')) == key]
                if twin and not named:
                    bad.append('no_profile: %s — профиль «%s» в базе УЖЕ ЕСТЬ (%s), '
                               'привяжите его' % (role, comps[twin[0]].get('name'), twin[0]))
