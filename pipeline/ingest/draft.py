...
WORDS = [
 ...
 ('Недвижимость',        r'бизнес-центр|\bбц\b|торгов\w+\s+центр|торгово-развлекательн|\bтрц\b|\bтрк\b'
                         ...
                         r'|\bга\s+земл|жил\w+\s+комплекс|\bжк\b|недвижим|имущественн\w+\s+комплекс'),
 ...
 ('Искусственный интеллект', r'искусственн\w+\s+интеллект|нейросет|машинн\w+\s+обучен|\bllm\b|генеративн'),
 ('ИТ и интернет',       r'\bит-|разработчик|программн\w+\s+обеспеч|\bсубд\b|облачн|кибербез|\bсофт\b|\bsaas\b'
 ...
 ('Строительство',       r'застройщик|строительн\w+\s+компан|подрядчик|цементн|бетон'),
 ('Агро',                r'агрохолдинг|агрокомплекс|сельхоз|птицефабрик|свиноком|молочн\w+\s+ферм|зернов|элеватор|тепличн|аквакультур|\bагро'),
 ('Пищепром и напитки',  r'пищев|кондитер|молочн|мясоперераб|мясн\w+\s+комбинат|пивовар|ликёро|спиртз|винодел|хлебокомбинат|хлебозавод|напитк'),
 ...
 ('Машиностроение',      r'машиностро|станкостро|приборостро|турбин|локомотив|судостро|верф|двигател'),
 ('Автопром',            r'автозавод|автопроизвод|автомобильн\w+\s+завод|автодилер|автосалон'),
 ('Транспорт и логистика', r'логистическ|грузоперев|транспортн\w+\s+компан|перевозчик|такси|каршеринг|авиакомпан|железнодорожн|аэропорт'),
 ('Порты и инфраструктура', r'\bпорт\b|портов|стивидор|терминал\w*\s+перевал'),
...
INDUSTRY_RX = [(ind, re.compile(p, re.I)) for ind, p in WORDS]
