...
STAGE_NEWS = re.compile(
    r'переговор|намерен|планир|подписал|заключил|согласован|одобрен|разрешен|разрешён|'
    r'закрыл|завершил|сделка\s+закрыта|не\s+состоял|отмен', re.I)
...
def match(item, idx):
    ...
    # Две совпавшие именованные сущности позволяют связать длинную сделку:
    # переговоры и закрытие нередко разделены больше чем 90 днями.
    for row in idx:
        gap = days_between(item.get('date'), row['date'])
        if len(t_quoted) >= 2 and t_quoted == row['quoted'] and gap <= 730:
            return row['id'], 'совпал набор названий в кавычках на разных этапах'
    # Более мягкое длинное окно допустимо только у открытой карточки и только
    # когда новая публикация явно сообщает этап, а не просто снова упоминает
    # одну и ту же компанию спустя год.
    if STAGE_NEWS.search(str(item.get('title') or '')):
        for row in idx:
            gap = days_between(item.get('date'), row['date'])
            if (row.get('status') in OPEN_STATUSES and t_quoted & row['quoted']
                    and len(t_stems & row['stems']) >= 2 and gap <= 365):
                return row['id'], 'открытая сделка: название в кавычках и этап в пределах года'
    for row in idx:
        gap = days_between(item.get('date'), row['date'])
