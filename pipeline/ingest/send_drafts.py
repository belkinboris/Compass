...
def build_plan():
    ...
    for d in latest_hold_drafts():
        ...
        # в hold-файле, если захочется поднять руками. Полностью собранный
        # черновик со сторонами латиницей ворота с 11 сентября 2026 держат
        # под ДРУГОЙ причиной («стороны названы латиницей…»), и она сюда не
        # попадает: PT/CyberOK прятался именно здесь (см. promote.py).
