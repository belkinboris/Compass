...
С 7 СЕНТЯБРЯ 2026 ЭТОТ СКРИПТ ПО УМОЛЧАНИЮ НЕ НУЖЕН И ВРЕДЕН. Сайт сам
забирает обновления опросом (`getUpdates`, см. `_poll_telegram_updates` в
main.py) и при старте СНИМАЕТ вебхук: Telegram не отдаёт обновления обоими
способами сразу. Запуск с `--write` вернёт вебхук, и опрос начнёт получать
отказ 409, то есть кнопки в консоли снова перестанут работать — до
следующей пересборки, которая вебхук опять снимет. Скрипт оставлен ради
пути назад: если опрос выключат (`TELEGRAM_POLLING=0`), вебхук снова станет
единственным входом, и регистрировать его придётся отсюда.
