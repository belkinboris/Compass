...
ЛЮБУЮ ещё не отправленную сделку — сегодня это больше тысячи карточек разом.
Без ограничения первый же прогон с настоящим токеном попытался бы выстрелить
их все подряд без пауз: Telegram отдаёт примерно 1 сообщение в секунду на
канал, и почти все запросы после первых нескольких десятков просто провалились
бы с ошибкой лимита — молча и без второй попытки. Это не решает, ЧТО делать со
старым бэклогом (нужно ли его вообще публиковать или «засеять» датой начала
публикации — решение владельца, см. PRODUCT_ROADMAP.md), но не даёт первому
включению токена случайно устроить ровно тот сценарий, которого боялись:
за один прогон уходит не больше `TELEGRAM_MAX_SENDS_PER_RUN` НОВЫХ постов
(по умолчанию 20 — при ежедневном притоке бэклог из тысячи разберётся за
несколько недель, а не за одну ошибочную минуту), и между отправками
выдерживается пауза `TELEGRAM_SEND_DELAY_S` (по умолчанию 1,2 с). Правки уже
опубликованных постов лимиту не подчиняются отдельно — они идут после новых
постов и делят с ними тот же общий лимит `TELEGRAM_MAX_SENDS_PER_RUN`.
