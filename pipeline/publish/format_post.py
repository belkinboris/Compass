...
def _sentences(text):
    ...
    ИСКЛЮЧЕНИЕ — многоточечное сокращение (инициалы, «B.V.», «J.P. Morgan»,
    «Л.Е.В. Менеджмент»): само правило «точка + пробел + заглавная» рвёт ЕГО
    ВНУТРИ, потому что последняя буква сокращения — заглавная и сама стоит
    перед точкой с пробелом. Найдено 18 сентября 2026 на посте о временном
    управлении «Ашаном» (карточка `g2daf32fe`): «Предмет: Auchan Russia —
    Менеджмент» также будет управлять 100%…» — от «АО «Л.Е.В. Менеджмент»»
    в посте осталось только «Менеджмент»», первая часть имени управляющего
    потерялась молча (`has_novelty` даже не увидела пропавший фрагмент —
    буквы сокращения короче двух знаков и не считаются «значащим словом»).
    Признак не спутать с обычным сокращением из одной точки («ВТБ.»,
    «EBITDA.», «IPO.») — там split нужен, это конец предложения: подавляем
    его, только если ПЕРЕД буквой стоит ЕЩЁ ОДНА точка (т. е. сокращение
    само состоит минимум из двух точечных сегментов). Замер по всей базе
    (deals, companies, pending) — 55 подавленных срабатываний, все —
    инициалы человека («В.П. Колбина», «С.Н. Шишкареву») или сокращения
    юрлица («B.V.», «N.V.», «A.D.», «J.P. Morgan», «G.O.I. ENERGY»),
    ложных нет; «ВТБ.»/«EBITDA.»/«IPO.» (одна точка, не подпадают под
    признак) по-прежнему честно разрезаются."""
    text = re.sub(r'\s+', ' ', str(text or '')).strip()
    if not text:
        return []
    parts = re.split(r'(?<=[.!?])(?<!\.[А-ЯЁA-Z]\.)\s+(?=[А-ЯЁA-Z])', text)
...
_FINANCIAL_INDICATOR = re.compile(
    ...
    r'|показател|маржа|маржи)', re.I)
_MONEY_AMOUNT = re.compile(
    r'\d[\d\s.,]*\s*(?:млн|млрд|тыс|трлн)\b|\d\s*(?:руб|₽)'
    # Финансовый коэффициент (маржа, ROE, рентабельность) выражается в
    # процентах, а не в рублях — «Рентабельность капитала… составила 23,8%»
    # не содержит ни одной суммы, но остаётся показателем из отчёта, а не
    # описанием компании. Найдено 11 сентября 2026 на карточке `c09e39da9`
    # («Абсолют Страхование») и на новой карточке о доразмещении акций
    # Совкомбанка: `_FINANCIAL_INDICATOR` слово находил («рентабельн»,
    # «маржа»), а `_MONEY_AMOUNT` требовал валюту и молчал — то же семейство
    # дефекта, что уже описано в докстринге ниже про «Базис»/Proto, просто
    # с процентом вместо суммы. Замер по всей базе: до правки 3 предложения
    # из `eco.target_fin` проходили мимо фильтра, у одной живой карточки
    # (`c09e39da9`) это было видно прямо в отрендеренном посте.
    r'|\d[\d,.]*\s*%', re.I)


_HEADCOUNT = re.compile(
    r'числ[а-яё]*\s+(?:работник|сотрудник)|среднесписочн', re.I)


def _is_financial_report(sentence):
    """Предложение только сообщает финансовый показатель с суммой — или
    численность персонала — а не описывает предмет сделки.
    ...
    Численность персонала («Средняя численность работников за 2025 год... —
    53 человека») — не сумма и не проходила ни `_FINANCIAL_INDICATOR`, ни
    `_MONEY_AMOUNT`, поэтому читалась как «новое про предмет» и попадала в
    «Предмет» тем же путём. Владелец, 11 сентября 2026 (карточка Трамплин
    Холдинг/«Мальт Систем»): «сколько сотрудников в компании — это не должно
    быть в предмете»."""
    if _HEADCOUNT.search(sentence):
        return True
...
# достичь 8 млрд рублей», «рынок оценивается в …». Строка «Зачем» в посте
# СНЯТА ЦЕЛИКОМ 11 сентября 2026 (владелец: она то есть, то нет — «получается
# дебильно» — и это неисправимо, пока мотив зависит от того, влез ли он в
# лимит строки), но `NOT_A_MOTIVE` остался: `accept_card.py` им проверяет,
# не начинается ли «Цель сделки» с оценки рынка вместо мотива, — это про
# качество самого поля карточки, не про строку поста.
...
def _buyer_label(deal):
    """«Покупатель» верно для обычной M&A, но не для временного управления по
    указу президента (Nestlé/«Ашан»/FM Logistic/«Лемана ПРО», 17 сентября
    2026): там нет покупателя — есть назначенный управляющий, а актив не
    продан, а изъят из управления прежнего владельца. Тот же класс дефекта,
    что уже чинился на сайте (static/index.html, `KIND_LABELS.custody`) —
    здесь свой, Python-side рендер поста, метку в нём никто не поменял
    вместе с сайтом. deal.get('kind') — то же поле, что читает и сайт;
    отдельный классификатор типа сделки (`dealKindOf` из static/index.html)
    сюда не переносим — он нужен только для остальных, уже названных
    кириллицей типов (СП, кредит, IPO…), а «custody» проставляется вручную
    и читается напрямую."""
