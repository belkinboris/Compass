...
OUTLET = (r"(?:Коммерсант[ъа-яё]*|Kommersant|Ведомост[а-яё]*|Vedomosti|РБК|RBC|Интерфакс[а-яё]*|Interfax"
          ...
          r"|Секрет\s+фирмы|Инк\.|Inc\.\s*Russia"
          # Агентство Бизнес Новостей нашлось в недельной очереди 18.09.2026
          # (юрист, сопровождавшая сделку Canpack, дала интервью изданию) —
          # ни полное имя, ни домен (TLD .agency не в списке DOMAIN) не
          # ложились на прежний список.
          r"|Агентство\s+Бизнес\s+Новостей|Abn\.Agency|Abn\s+Agency"
          r"|Ъ)")
...
JARGON = [
    # «штамп» отделён от остальных: у нас это слово диалектное («поставить
    # штамп»), но в промышленности оно настоящее — «штамповая оснастка»,
    # «штамповочные операции», «Штамповые технологии». Поэтому здесь только
    # формы существительного, а всё, что продолжается на «ов…», «очн…»,
    # «ованн…», правилом не считается (замер по базе: три ложных находки).
    re.compile(r'\b(?:знаменател|линз[аеыу]|дочитыван|дочита[лн]|инвариант|идемпотент|прогон)[а-яё]*'
               r'|\bштамп(?:[аеыу]|ом|ов|ами|ах)?(?![а-яё])|\bbulk\b', re.I),
...
# Пометка о том, ГДЕ МЫ ЭТО ПРОЧИТАЛИ, — внутренняя кухня, и в тексте для
# читателя её быть не должно нигде: ни в карточке, ни в описании компании, ни
# в посте («ВИМ Сбережения — управляющая компания фонда (подтверждено чтением
# источника mergers.ru)», 22 сентября 2026). Это УЖЕ правило press_attribution,
# но здесь взяты только те его признаки, которые не могут оказаться фактом о
# самой компании: слово «издание» законно стоит у РБК и Rusbase, а «(по данным
# TAdviser)» не законно нигде.
SOURCE_NOTE_RULES = ('издание в скобках', 'пресс-атрибуция: оборот + издание',
                     'пресс-атрибуция: издание + оборот', '«Источник:» в тексте')
INTERNAL_NOTE = re.compile(
    r'подтвержд[а-яё]*\s+чтением|по\s+наш(?:им|ей)\s+(?:данным|базе)'
    r'|\bиз\s+нашей\s+базы\b|втор\w+\s+источник\w*\s+это\s+не\s+подтвержда'
    r'|по\s+данным\s+одного\s+источника', re.I)


def source_note(text):
    """Пометки о происхождении сведений в тексте, который читает человек.

    Возвращает список (правило, найденный кусок) — пусто, если чисто. Уже,
    чем press_attribution: держит забор по всей базе, поэтому не должен
    краснеть на медиахолдинге, который по роду занятий и есть издание.
    """
    found = [(n, frag) for n, frag in press_attribution(text) if n in SOURCE_NOTE_RULES]
    m = INTERNAL_NOTE.search(str(text or ''))
    if m:
        found.append(('пометка о нашей работе', m.group(0).strip()))
    return found
