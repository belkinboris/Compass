...
одновременно даёт результат и подтверждает его: так порог «доля 95% и выше»
пропускал 30% Guess и 49% «Полиматики» (доля бралась максимумом из названных
процентов), а тест на синтетических примерах был зелёным.

С 6 сентября 2026 обе эти сделки в выборке стоят как ПРОХОДЯЩИЕ — но по
другой причине и с другим числителем: цена пакета делится на его долю и
сравнивается с показателем всей компании (`price_scaled` в строке выборки).
Порог остался, только он теперь про осмысленность пересчёта, а не про
контроль: доли меньше четверти не пересчитываются.
...
def test_gold_price_is_scaled_exactly_where_the_sample_says_so(row):
    """`price_scaled` в выборке — утверждение читателя «в карточке цена
    ПАКЕТА»; код обязан именно её делить на долю, а не брать как есть.
    Обратное тоже проверяется: где выборка молчит, пересчёта быть не должно."""
    _pending(row)
    d = DEALS[row['id']]
    cand, _ = dm.admission(dict(d, id=row['id']), CONFIRMED, BANKS, LOTS)
    if not cand:
        pytest.skip('сделка не проходит текстовые правила — числителя нет')
    value, basis = dm.implied_full_price(cand.sum_rub, cand.stake_percent, cand.price_scope)
    assert (basis == 'scaled') == bool(row.get('price_scaled')), (row['id'], basis, row['why'])
    if basis == 'scaled':
        assert value == pytest.approx(cand.sum_rub * 100.0 / row['share']), row['id']
