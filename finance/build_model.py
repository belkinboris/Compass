...
def sheet_assumptions(wb):
    ...
    ref["dev_free"] = row("Первые месяцы без разработчика", 6, "Ваше решение.", RUB0)
...
def sheet_calc(wb, ref):
    ...
    line("Разработчик", lambda i: "=IF(%d<=%s,0,IF(%d<=%s,%s,%s))"
         ...
         note="Шесть месяцев без разработчика, шесть по 50 тыс, дальше по 100 тыс.")
...
def sheet_tax_risk(wb, ref):
    ...
    paid_out = "(%s*6+%s*12)" % (A("dev1_cost"), A("dev2_cost"))
    for name, formula, note in (
        ("Выплачено разработчику за 24 месяца", "=%s" % paid_out,
         "Шесть месяцев по первой ставке и двенадцать по второй."),
