...
# Оба тарифа названы владельцем: One 990 ₽ в месяц, Pro 49 900 ₽ в год.
...
# 49 900 съедала бы 36% чека. Считаем от измеренного — команда из трёх
...
print("решение поставить 49,9 тыс, а не больше:")
for price in (49900, 90000, 150000):
...
print("=" * 78)
print("Главный вопрос к цене 49 900 ₽: один договор Pro стоит около четырёх")
print("подписок One, то есть это не рычаг, а второй ручеёк. Значит, всё решает")
...
def _first_plus(pace, mkt=None, dev=True):
    global FIX, DEV1_COST, DEV2_COST
    was, d1, d2 = dict(FIX), DEV1_COST, DEV2_COST
    if mkt is not None:
        FIX = dict(FIX); FIX["mkt"] = mkt
    if not dev:
        DEV1_COST = DEV2_COST = 0
    _p = dict(SCEN["1 в месяц"]); _p["corp_per_month"] = float(pace)
    _, _r = run("1 в месяц", _p)
    FIX, DEV1_COST, DEV2_COST = was, d1, d2
    return next((x["m"] for x in _r if x["net"] > 0), None)


_with_dev = next((x for x in (1, 2, 3, 4, 5) if _first_plus(x)), None)
_no_dev = next((x for x in (0.5, 1, 2, 3) if _first_plus(x, mkt=0, dev=False)), None)
print()
_n = _with_dev * 12
_word = "договор" if _n % 10 == 1 and _n % 100 != 11 else (
    "договора" if _n % 10 in (2, 3, 4) and _n % 100 not in (12, 13, 14) else "договоров")
print("  ВЫВОД. С разработчиком нужно %d Pro в месяц — это %d %s за год."
      % (_with_dev, _n, _word))
print("  Без разработчика и без платного продвижения хватает %s в месяц, и плюс"
      % ("%.1f" % _no_dev).replace(".", ","))
print("  приходит на %d-м месяце." % _first_plus(_no_dev, mkt=0, dev=False))
print("  То есть при цене %s Pro не оплачивает разработчика — он оплачивает"
      % money(HUMAN["corp_price"]))
