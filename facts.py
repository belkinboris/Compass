...
REASON_LABELS = {
    ...
    # Пятый разбор рецензента (7 сентября 2026): отказ по периметру — не
    # пробел в данных, а полезный профессиональному читателю ответ, если
    # назвать ПРИЧИНУ, а не признак. «Предмет — лот из нескольких юрлиц»
    # говорит о нашей разметке; «куплено несколько компаний, а отчётность
    # покрывает не весь периметр» объясняет, почему делить цену на найденную
    # выручку нельзя.
    'target_lot': 'куплено несколько компаний, отчётность покрывает не весь периметр сделки',
    'target_bank': 'предмет — банк',
    'target_unconfirmed': 'у предмета не подтверждён ИНН',
    'perimeter_not_verified': 'не проверено, покрывает ли отчётность купленной компании всю сделку',
    ...
    'perimeter_report_missing': 'периметр подтверждён, но без привязки к конкретному отчёту',
