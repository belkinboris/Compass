...
def test_filter_rejects_military_arms_sales_and_drug_crime_statistics():
    """Найдено 16 сентября 2026: владелец увидел в консоли «сомнительных»
    «WP узнала о продаже США десятков тысяч «противоречивых» авиабомб
    Израилю» и «В России доля приговоров за приобретение мефедрона и
    альфа-PVP выросла до 50%» — оба прошли POSITIVE («продаже»,
    «приобретен»), а предмет-компанию `draft.py` честно не смог разобрать
    (это не M&A-сделка), и черновик лёг в hold видимым «сомнительным», а не
    молча отсеялся. Проверяем, что оба слова остаются рабочими для
    настоящих сделок (продажа бизнеса, приобретение доли)."""
    assert not classify.looks_like_deal(
        'WP узнала о продаже США десятков тысяч «противоречивых» авиабомб Израилю')
    assert not classify.looks_like_deal(
        'В России доля приговоров за приобретение мефедрона и альфа-PVP выросла до 50%')
    assert classify.looks_like_deal('«Лента» ведёт переговоры о продаже сети магазинов')
    assert classify.looks_like_deal('Фонд приобрёл долю в разработчике софта')
    # Танкер — профильный актив (морской транспорт), не военная техника;
    # граница у MILITARY исключает его явно (?!ер) после «танк».
    assert classify.looks_like_deal('Совкомфлот продал танкер за $40 млн')
...
def test_post_never_prints_a_buyer_line_without_a_buyer_name():
    """Найдено владельцем 25 августа (Pridex/Multispace, ge283bafc): без
    имени покупателя `_party_detail()` всё равно подтягивал `eco.context`
    целиком, и пост выходил со строкой «Покупатель: В периметр сделки
    вошли четыре объекта…» — предложение вообще не о покупателе. Деталь
    без имени не идентифицирует сторону: строка «Покупатель» обязана
    нести хотя бы имя, деталь — только довесок к нему, никогда замена."""
    deal = {
        "id": "ge283bafc", "title": "Pridex продал сеть сервисных офисов Multispace",
        "status": "Закрыта", "date": "2026-08-24", "seller": "Pridex",
        "eco": {"context": "В периметр сделки вошли четыре объекта общей площадью "
...
def test_post_attaches_buyer_detail_only_alongside_the_buyer_name():
    """Тот же механизм, положительный случай: имя покупателя есть — деталь
    из `eco.context` законно идёт довеском. Само имя перед тире с 4 сентября
    2026 не дублируется, когда деталь называет покупателя первой же (см.
    `names_party_upfront`): строка несёт имя ровно один раз."""
    deal = {
        "id": "gx1", "title": "«Ромашка» купила «Одуванчик»",
        "buyer": "b1", "status": "Закрыта", "date": "2026-08-24",
        "eco": {"context": "«Ромашка» — крупный региональный девелопер жилой недвижимости."},
    }
...
def test_post_buyer_name_is_not_printed_twice_when_the_detail_already_says_it():
    """Найдено владельцем 4 сентября 2026 на живом посте о ТЦ «Город Косино»
    (gddb34475): «Покупатель: ООО «Афкап» — Новым владельцем компании стало
    ООО «Афкап» Агиля Мохнатова…» — «повторение странное». Тот же дефект, что
    2 сентября чинили в строке «Предмет» (госпакет Шереметьево), только там
    хватило мягкой сверки по основам слов, а здесь она ОПАСНА, и тест держит
    обе стороны границы (замер по всем 1589 карточкам: мягкое правило меняло
    65 строк и ломало 48 из них).
    """
    # 1. Деталь называет покупателя первой же фразой — имя перед тире лишнее.
    deal = {
        "id": "gddb34475", "title": "«Газпромбанк-инвест» продал ТЦ «Город Косино»",
        "buyer_name": "ООО «Афкап»", "status": "Закрыта", "date": "2026-09-02",
        "eco": {"context": "Новым владельцем компании стало ООО «Афкап» Агиля "
    ...
    #    на «займы»): имя обязано остаться, иначе читатель не узнает покупателя
    #    вовсе — ровно дефект Pridex/Multispace, 25 августа.
    deal_stem = {
        "id": "gx-stem", "title": "Продан сервис онлайн-кредитования",
        "buyer_name": "Займер", "status": "Закрыта", "date": "2026-09-02",
    ...
    #    без имени впереди строка читалась бы как «покупатель — DocMed».
    deal_tail = {
        "id": "gx-tail", "title": "Клиника DocMed привлекла инвестиции",
        "buyer_name": "«ТилТех Капитал»", "status": "Закрыта", "date": "2026-09-02",
        "eco": {"context": "Московская сеть клиник доказательной медицины DocMed "
    ...
    assert format_post.names_party_upfront("ООО «Афкап»", "Новым владельцем стало ООО «Афкап» Агиля")
    assert not format_post.names_party_upfront("Займер", "Микрофинансовая компания: онлайн-займы.")
    assert not format_post.names_party_upfront(
        "«ТилТех Капитал»",
        "Московская сеть клиник доказательной медицины DocMed привлекла 200 млн ₽ "
        "от инвестиционного фонда «ТилТех Капитал».")
    # Разнобой кавычек в базе не должен мешать дословной сверке.
    assert format_post.names_party_upfront('ООО "Афкап"', 'Новым владельцем стало ООО «Афкап».')


def test_post_buyer_detail_skips_the_sentence_about_who_reported_the_deal():
    """Найдено владельцем 30 августа на живом посте «Транснефть»/Газпромбанк
    (g60f99ba6): «Покупатель: «Транснефть» — О покупке ... сообщила в
    финансовой отчетности по МСФО...» — предложение о том, КТО И ГДЕ СООБЩИЛ,
    а не о покупателе. Оно ещё и выигрывало приоритет «имя в кавычках»,
    потому что имя стороны стояло в кавычках именно в нём. Мета-предложение
    о раскрытии не годится в пояснение стороны никогда."""
    deal = {
        "id": "gx-meta", "title": "«Транснефть» приобрела 7,52% акций Газпромбанка",
        "buyer_name": "«Транснефть»", "status": "Закрыта", "date": "2026-08-28",
        "eco": {"context": "О покупке 7,52% голосующих ценных бумаг одной из компаний "
                            "«Транснефть» сообщила в финансовой отчетности по МСФО за "
                            "первое полугодие 2026 года. Стоимость сделки составила "
                            "40 млрд руб."},
    }
    text = format_post.render(deal, {})
...
def test_post_buyer_detail_requires_a_sentence_about_the_buyer_itself():
    """Замер 30 августа по всем 1575 карточкам: у 93 постов к имени покупателя
    клеилось предложение о ЦЕЛИ или ПРОДАВЦЕ («Покупатель: Иван Тырышкин —
    PPF Group получила 318 млн евро убытка»). Предложение из `eco.context`
    может пояснять сторону, только если называет ЕЁ; описание профиля (`desc`)
    проверки не требует — оно по построению о компании."""
    deal = {
        "id": "gx-alien", "title": "Завод «Прогресс» продан частному инвестору",
        "buyer_name": "Иван Тырышкин", "status": "Закрыта", "date": "2026-08-24",
        "eco": {"context": "PPF Group в первом полугодии 2022 года получила 318 млн "
...
def test_reporting_meta_needs_both_a_verb_and_a_channel():
    """Защита от перефильтровки: «сообщила, что покупка усилит позиции» —
    содержание, а не мета (глагол есть, канала раскрытия нет). Фильтр требует
    обоих признаков сразу."""
    assert format_post._is_reporting_meta(
        "О покупке доли компания сообщила в финансовой отчетности по МСФО.")
    assert format_post._is_reporting_meta(
        "Сделка закрыта в марте, говорится в пресс-релизе покупателя.")
    assert not format_post._is_reporting_meta(
        "Компания сообщила, что покупка усилит её позиции на рынке.")
    assert not format_post._is_reporting_meta(
        "Выручка завода в 2025 году составила 4 млрд рублей.")


def test_post_subject_line_names_the_subject_not_its_revenue():
    """Найдено владельцем 7 сентября 2026 на черновике поста «Базис»/Proto:
    «Предмет: По данным Saby Profile, доходы ООО «ПротоСервисез» … составили
    9,5 млн руб.» — вместо предмета в строке стояли его финансовые
    показатели. Причина — приоритет источников: при пустом «Предмет / доля»
    деталь берётся из «Финансов предмета», а там первое предложение почти
    всегда числовое. Из «Финансов предмета» в «Предмет» идёт только то
    предложение, которое НАЗЫВАЕТ предмет."""
    deal = {
        "id": "g95370469", "title": "«Базис» купил 70% компании Proto",
        "status": "Закрыта", "date": "2026-09-07", "target": "t1",
        "eco": {"target_fin": "По данным Saby Profile, доходы ООО «ПротоСервисез» "
                              "по итогам 2025 г. составили 9,5 млн руб., чистая "
                              "прибыль – 4,5 млн руб."},
    }
    ...
    # по-прежнему годится: ради него источник в приоритеты и попал (карточка-
    # образец HeadHunter/Happy Job).
    named = dict(deal, target=None, eco={
        "target_fin": "Выручка выросла до 4 млрд ₽. Покупается 51% ООО «Счастливая работа»."})
    assert "51% ООО «Счастливая работа»" in format_post.render(named, {})


def test_post_subject_sentences_from_different_sources_get_a_period_between_them():
    """Найдено 30 августа на «Транснефть»/Газпромбанк (g60f99ba6): `eco.share`
    — короткая фраза без точки на конце, а не предложение. Когда предмет
    (`asset`) уже назван в заголовке и не новый, `_subject_detail()` берёт
    ВТОРОЙ источник — профиль цели (`desc`) — и раньше `' '.join(...)` клеил
    его к `eco.share` встык: «...ценных бумаг Опорный банк газовой отрасли»
    читалось как один сломанный кусок текста. Между двумя источниками
    обязана стоять точка, а не пробел."""
    deal = {
        "id": "g60f99ba6", "title": "«Транснефть» приобрела 7,52% акций Газпромбанка",
        "status": "Закрыта", "date": "2026-08-28", "target": "t1",
        "eco": {"share": "7,52% голосующих ценных бумаг"},
    }
    companies = {"t1": {"name": "Газпромбанк",
                        "desc": "Опорный банк газовой отрасли: корпоративное кредитование."}}
    text = format_post.render(deal, companies)
    assert "ценных бумаг Опорный" not in text, "два источника склеены без знака препинания"
    assert "ценных бумаг. Опорный банк газовой отрасли" in text


def test_sentence_split_does_not_break_multi_dot_abbreviations():
    """18 сентября 2026, карточка «Ашан»/временное управление (`g2daf32fe`):
    `_sentences()` резала «точка + пробел + заглавная», и «АО «Л.Е.В.
    Менеджмент» также будет управлять...» разваливалось на «Л.Е.В.» и
    «Менеджмент» также будет...» — в посте от имени управляющего осталась
    только половина («Предмет: Auchan Russia — Менеджмент» также будет
    управлять...»). Причина глубже одной карточки: `_significant_words()`
    режет текст на буквы по одной (периоды — не часть слова), и однобуквенные
    «Л», «Е», «В» короче двух знаков и не считаются значимыми — фрагмент
    «Л.Е.В.» терял всякую «новизну» и вымывался целиком. Починка — не
    склеивать «X.Y.» с СЕДУЮЩИМ предложением, если букве перед точкой
    предшествует ЕЩЁ ОДНА точка (иначе обычный акроним вроде «ВТБ.»/«IPO.»
    тоже перестал бы завершать предложение — а он должен). Замер по всей
    базе (55 случаев: инициалы человека, «B.V.», «J.P. Morgan», «A.D.») —
    ни одного ложного срабатывания."""
    text = ('«Л.Е.В. Менеджмент» также будет управлять 100% ООО «Ашан». '
            'Сумма сделки не раскрывается.')
    sentences = format_post._sentences(text)
    assert sentences[0].startswith('«Л.Е.В. Менеджмент»'), \
        "сокращение «Л.Е.В.» оторвано от своего предложения: %r" % sentences
    # Обычный акроним из одной точки по-прежнему завершает предложение —
    # иначе правило просто перестало бы резать вообще что-либо.
    ordinary = format_post._sentences('Финансирование обеспечил ВТБ. Стратегическая цель — консолидация рынка.')
    assert ordinary == ['Финансирование обеспечил ВТБ.', 'Стратегическая цель — консолидация рынка.']


def test_buyer_label_is_not_pokupatel_for_custody_deals():
    """Тот же случай, что и выше, но про подпись строки, а не про обрыв
    текста: «Покупатель» неверно для временного управления по указу
    президента — покупателя там нет, есть назначенный управляющий (owner,
    18 сентября 2026: «когда национализируют активы там нет покупателя и
    продавца»). `d.kind` — то же поле, что читает и `static/index.html`."""
    assert format_post._buyer_label({'kind': 'custody'}) == 'Временный управляющий'
    assert format_post._buyer_label({'kind': 'acquisition'}) == 'Покупатель'
    assert format_post._buyer_label({}) == 'Покупатель'
...
def test_enrich_does_not_restore_a_retracted_value(base):
    """Поле пусто НЕ потому, что его никогда не проверяли, а потому, что
    значение сознательно СНЯЛИ (18 сентября 2026, g5599430f: «2,05 млрд ₽» —
    уставный капитал SPV, совпавший по величине с ценой в заголовке источника,
    а не цена сделки; та же статья вернулась в ленте назавтра и тихо вернула
    ту же ошибку дважды подряд, пока `retracted` не появился). Повторная
    догадка того же значения обязана уйти человеку («расхождение»), а не
    тихо записаться в базу заново."""
    import enrich
    deal = next(d for d in base["deals"] if not format_post.has(d.get("sum")))
    deal = dict(deal, retracted={"sum": ["2,05 млрд ₽"]})
    item = {"title": "Стороны подтвердили сумму сделки — 2,05 млрд рублей",
            "url": "https://example.invalid/enrich-retracted", "source_id": "web:kommersant.ru"}
    props = enrich.proposals(deal, item, {}, base["companies"])
    kinds = {field: kind for field, _v, kind, _w in props}
    assert kinds.get("sum") == "расхождение", props

    # Другое, не снятое значение того же поля пустой карточки всё ещё
    # дописывается как обычно — retracted не глушит поле целиком.
    item2 = {"title": "Стороны подтвердили сумму сделки — 900 млн рублей",
             "url": "https://example.invalid/enrich-retracted-2", "source_id": "web:kommersant.ru"}
    props2 = enrich.proposals(deal, item2, {}, base["companies"])
    kinds2 = {field: kind for field, _v, kind, _w in props2}
    assert kinds2.get("sum") == "добавить", props2
...
def test_main_dry_run_with_a_real_token_prints_full_post_text(monkeypatch, tmp_path, capsys):
    ...
    deal = {"id": "gX1", "title": "«Ромашка» купила «Одуванчик»",
            ...
            "eco": {"context": "«Ромашка» работает на рынке коммерческой недвижимости "
...
def test_main_never_posts_a_stale_deal_on_first_encounter(monkeypatch, tmp_path, capsys):
    """5 сентября 2026, правило владельца: «не нужно публиковать в тг не новые
    сделки, даже если карточка появляется новая про старую сделку» —
    подписчики, которые следят за настоящими новостями, отписываются, увидев
    архив, даже честно подписанный «Сделка из базы · Публикуем впервые».
    Повод — пост про «МорТехПром» с такой шапкой (сама та сделка оказалась
    свежей, дата стояла годом без месяца — это починено отдельно в
    format_post.is_fresh). Не первой свежести сделка, впервые встреченная
    прогоном, теперь молча засевается (как решение модерации «без поста»), а
    не отправляется вовсе. Тестовая карточка — архивная: год без месяца и без
    признака притока."""
...
def test_old_deal_is_posted_as_news_about_a_known_deal():
    ...
    # …кроме карточки, которую приток завёл на этой неделе из свежей новости
    # (5 сентября 2026, «МорТехПром»: день сделки в статье не назван, но
    # новость свежая) — свежесть считается по `added`. Архивная карточка (без
    # from_ingest), давно добавленная или с другим годом под это не попадает.
    ingest = dict(old, date="2026", from_ingest=True, added="2026-08-03")
    assert "Сделка из базы" not in format_post.render(ingest, {}, today=today)
...
def test_leading_share_does_not_eat_the_start_of_a_longer_word():
    """`LEADING_SHARE` резал ведущее слово («долю», «пакет», «контроль») без
    границы слова — и «контроль» матчился ПРЕФИКСОМ «контрольную», обрезая
    хвост до «ную долю…». Найдено на карточке НМГ/«Комсомольская правда»
    (3 сентября 2026): «НМГ приобретает контрольную долю издательского дома
    «Комсомольская правда»» разбиралось предметом «ную долю издательского
    дома «Комсомольская правда»» — потерянные первые восемь знаков молча
    портили карточку. `\\b` после словесных вариантов чинит это, не трогая
    настоящую числовую форму («25% акций»), где после «%» границы слова нет
    и `\\b` там не сработал бы вовсе.
    """
    import draft
    buyer, asset, seller, _ = draft.guess_parties(
        'НМГ приобретает контрольную долю издательского дома «Комсомольская правда»')
    assert buyer == 'НМГ'
    assert asset == 'контрольная доля издательского дома «Комсомольская правда»'
    # Настоящие ведущие слова по-прежнему режутся.
    buyer, asset, seller, _ = draft.guess_parties('Сбербанк купил долю в ООО «Ромашка»')
    assert asset == 'ООО «Ромашка»'
    buyer, asset, seller, _ = draft.guess_parties('Фонд купил 25% акций компании «Восход»')
    assert asset == '«Восход»'
...
def test_gate_holds_a_deal_with_no_russian_connection(base):
    ...
    for foreign in ("Visa купила BioCatch за $2,4 млрд для защиты от кибермошенничества",
                    ...
                    # 17–18 сентября 2026, третий день с одной и той же
                    # историей у разных изданий: Тодд Бёли и Марк Уолтер
                    # продали долю в лондонском «Челси» Clearlake Capital
                    # Group. «Челси» (клуб, родня «Ливерпуля»), «Бёли» и
                    # «Тодд» (фамилия и имя продавца) снимались по одному —
                    # та же конструктивная неполнота списков, что и у
                    # Цукерберга/Странкалли/Ирландии.
                    "Тодд Бёли продал свою долю в «Челси»",
                    "Американский предприниматель Тодд Бёли продал свою долю в клубе «Челси»",
                    # Тот же час, четвёртый кандидат: британский телеведущий
                    # распродаёт личную коллекцию машин через британский
                    # аукционный дом — ни одного российского признака, кроме
                    # транслитерированного имени «Джереми».
                    "Экс-ведущий Top Gear Джереми Кларксон продаст три авто из своей коллекции"):
...
def test_console_does_not_ask_about_a_post_that_will_never_go_out(monkeypatch):
    ...
    для неё бессмыслен: вопрос, ответ на который ничего не решает (повод —
    «МорТехПром»: пост получил в консоли шапку архива, был одобрен кнопкой и
    так и ушёл). Карточка 🗂 уходит как обычно и сама говорит, что канал
...
def test_post_proofreading_catches_a_sentence_masquerading_as_a_party_name():
    """25 августа владелец увидел в канале «Покупатель: В периметр сделки
    вошли четыре объекта…» — целое предложение-описание вместо имени
    (Pridex/Multispace, ge283bafc). is_generic_description() его не ловит:
    он ищет родовое слово («Группа»/«Компания») В НАЧАЛЕ значения, а тут в
    начале обычный предлог. Правило иное — предлог в начале И точка в
    конце — проверено на всей базе (1436 строк Продавец/Покупатель, ни
    одного ложного срабатывания) отдельным замером до внедрения."""
    import check_post
    bad = ('<b>X</b>\n\nПокупатель: В периметр сделки вошли четыре объекта '
           'общей площадью 18,2 тысячи квадратных метров в Москве.')
    assert any('предложение-описание' in p for p in check_post.check(bad))
    # Деталь через тире законно бывает целым предложением — с именем ДО
    # тире правило молчит, проверяется только часть до тире.
    with_detail = ('<b>X</b>\n\nПокупатель: «Ромашка» — В 2024 году компания '
                  'вышла на рынок Санкт-Петербурга.')
    assert not check_post.check(with_detail), check_post.check(with_detail)
...
def test_promote_holds_the_same_deal_told_from_the_other_side(base):
    """Продавец и покупатель об одной сделке — не две карточки.

    Владелец 8 сентября 2026 открыл карточку «Очень плохая карта про тц родник
    и алмаз» и оказался прав дважды: карточка была бедной И была дублем. В базе
    с 31 июля лежала подробная запись той же сделки, написанная со стороны
    продавца («ПСБ выставил на продажу компании, владеющие двумя ТРК в
    Челябинске»), а приток 7 сентября завёл вторую — со стороны покупателя
    («…купил ТЦ «Родник» и «Алмаз»…»). Ни одна проверка на дубли этого не
    увидела, потому что все они читали ЗАГОЛОВОК против ЗАГОЛОВКА: общее слово
    там одно («челяби»), а у продавцовой формулировки в заголовке нет ни одного
    названия в кавычках — имена обоих ТРК и обоих проданных юрлиц стоят в
    предмете и в «Предмете / доле».

    Правило сверяет имена из заголовка новой карточки с именами из ТЕЛА старой
    и требует ДВУХ совпадений: одного мало (замер по базе — 22 пары, почти все
    серийные игроки вроде банка «Траст»), двух — 5 пар за три года.

    Индекс здесь синтетический, без настоящей базы: сама сделка после слияния
    дубля живёт в базе с обоими названиями уже в ЗАГОЛОВКЕ, и на ней сработало
    бы старое правило — тест проверял бы не то, что чинили.
    """
    import promote
    existing = {
        "id": "gtest-trk", "date": "2026-08-17",
        "title": "ПСБ выставил на продажу компании, владеющие двумя ТРК в Челябинске",
        "ind": "Недвижимость", "type": "Продажа с торгов", "status": "Закрыта",
        "asset": "ООО «Родник» и ООО «Управляющая компания «Содействие»»",
        "eco": {"share": "ПСБ выставил на аукцион 100% уставного капитала ООО «Родник» "
                         "и ООО «Управляющая компания «Содействие»», управляющих "
                         "комплексами «Родник» и «Алмаз» в Челябинске."},
        "src": [["источник", "https://example.invalid/trk"]]}
    idx = matcher.index_base([existing])
    df = promote.stem_frequency(idx)
    twin = {"title": "«Центр инжиниринговых услуг при проектировании и строительстве» "
                     "купил ТЦ «Родник» и «Алмаз» в Челябинске за 6 млрд рублей",
            "date": "2026-08-06"}
    found = promote.near_duplicate(twin, idx, df)
    assert found and found[0] == "gtest-trk", (
        "карточка о той же сделке со стороны покупателя прошла как новая: %r" % (found,))
    assert "предмете" in found[1], (
        "сработало не то правило, ради которого писался тест: %r" % (found[1],))

    # Правило проверено на себе: ОДНОГО общего имени мало. Иначе банк «Траст»,
    # продающий десятки активов, слипался бы сам с собой каждую неделю (замер
    # 8 сентября 2026: одно имя — 22 пары по базе, два — 5).
    one_name = {"title": "Инвестор купил комплекс «Алмаз» в другом городе",
                "date": "2026-09-05"}
