...
def test_filter_rejects_official_summary_of_seized_assets():
    """Найдено 18 сентября 2026, дважды за один день на живом потоке: ворота
    пропустили в предпросмотр сводную реплику чиновника о проданном за
    период имуществе — «Украина продает имущество в Харькове по заниженной
    цене, заявил Ганчев» (общая жалоба на практику, ни один объект/сумма/
    покупатель не назван) и «Крым продал десять активов украинских
    олигархов, сообщил Константинов» (сводная цифра за период, покупатель
    не назван ни для одного из десяти активов). Разбор оба раза принял
    регион/страну за продавца, а хвост заголовка — за предмет сделки.
    Настоящая сделка с этими регионами как источником новости (не как
    продавцом общим словом) должна по-прежнему проходить фильтр."""
    assert not classify.looks_like_deal(
        'Украина продает имущество в Харькове по заниженной цене, заявил Ганчев')
    assert not classify.looks_like_deal(
        'Крым продал десять активов украинских олигархов, сообщил Константинов')
    assert classify.looks_like_deal(
        'Крымский завод продал 25% акций компании «Волна», сообщил представитель холдинга')
...
def test_a_transfer_to_state_management_is_recognised_in_the_active_voice_too():
    """«Путин передал во временное управление» — такая же сделка, как «переданы».

    22 сентября 2026 владелец спросил, почему в базе нет «Аптар Владимир»:
    президент передал компанию во временное управление, новость была в лентах,
    а карточки нет. Причина — фильтр знал только СТРАДАТЕЛЬНЫЙ оборот
    («активы переданы во временное управление»), а ленты пишут
    действительным: «Путин передал…». CanPack в базе есть ровно потому, что
    тот заголовок оказался страдательным.

    Классический случай «правило написали по одному примеру и оно не
    работает на соседнем» — тот же класс, что падежи и `\\b` в регулярках.

    Вторая половина теста важнее первой: «внешнее управление» — это ещё и
    процедура банкротства по 127-ФЗ, и такие заголовки сделкой НЕ являются.
    Граница держится обеими сторонами, иначе, расширив правило, мы впустим
    в приток все банкротства страны.
    """
    sys.path.insert(0, str(ROOT / "pipeline" / "ingest"))
    import classify

    for title in ("Путин передал во временное управление Росимущества компанию «Аптар Владимир»",
                  "Путин подписал указ о передаче во временное управление активов «Аптар Владимир»",
                  "Российские активы Aptar переданы во временное управление",
                  "Путин передал во временное управление российские активы CanPack",
                  "Путин подписал указ о временном управлении активами Uniper и Fortum",
                  "Владимир Путин передал во внешнее управление «Аптар Владимир»"):
        assert classify.looks_like_deal(title, ""), title

    for title in ("Арбитражный суд ввёл внешнее управление в ООО «Ромашка»",
                  "Собрание кредиторов продлило внешнее управление на полгода",
...
def test_a_named_legal_entity_cannot_be_postponed_as_missing_a_profile():
    """«Профиля ещё нет» — не причина отложить, когда юрлицо уже названо.

    Владелец 23 сентября 2026: «мы же на этапе черновика уже можем найти
    профиль и посмотреть отчётность». На карточке «Лента»/«Мария-Ра»
    предмет стоял текстом «ООО «Розница К-1»», читатель отложил профиль с
    причиной «профиля в базе ещё нет» — и пост вышел без финансов предмета,
    хотя имя юрлица было прямо в карточке, а по нему ищется ИНН и отчётность.

    Откладывать по-прежнему можно лот, список сторон и физлицо: замер по базе
    в тот же день дал 29 отложенных профилей, юрлицо названо у четырёх, и у
    двух из них откладывать правильно.
    """
    import accept_card

    single = accept_card.single_legal_entity
    assert single('ООО «Розница К-1»') == 'ООО «Розница К-1»'
    # вложенные кавычки — всё ещё одно имя
    assert single('ООО «Кинокомпания «Парадиз продакшнз»»')
    # перечисления и физлица откладывать можно
    assert single('АО «Норд-Вест СВ», «Инструмент-СВ», «Машиностроение СВ»') is None
    assert single('ООО «Некстеп», Теймур Штернлиб и Дмитрий Васильев') is None
    assert single('ПАО «Лента» и ООО «Монетка»') is None
    assert single('Александр Ракшин') is None
    assert single('бизнес-центр на Арбате') is None
    assert single('') is None

    base = {'companies': {}, 'deals': [], 'telegram_posts': {}}
    card = {'id': 'gtestlegal', 'title': 'x', 'type': 'M&A', 'status': 'Обсуждается',
            'asset': 'ООО «Розница К-1»', 'seller': 'Александр Ракшин',
            'src': [['Retailer.ru', 'https://retailer.ru/x']]}
    ans = {'id': 'gtestlegal', 'checklist': {}, 'profiles': [],
           'no_profile': {'target': 'профиля в базе ещё нет',
                          'seller': 'физлицо, профиля нет'}}
    bad = accept_card.check_answer(ans, card, base)
