...
def test_plastic_packaging_is_chemicals_not_food(base):
    """Упаковка для еды — не производство еды.

    Слово «пищевой» в «пластиковой пищевой упаковки» уводило отрасль в
    «Пищепром и напитки». Материал решает: по пластику в базе два прецедента и
    оба «Химия и удобрения» (переработка пластика у «Технониколь», БОПП-плёнка
    Manucor). Бумажная упаковка сюда НЕ добавлена — там наша собственная
    разметка расходится, и правило выше согласованности меток не прыгнет.
    """
    import draft
    for phrase in ("выпускает в Уфе пластиковую упаковку",
                   "российский производитель пластиковой пищевой упаковки",
                   "завод по переработке пластика в Твери",
                   "производителя БОПП-пленки Manucor"):
        assert draft.industry_by_words(phrase) == "Химия и удобрения", phrase
