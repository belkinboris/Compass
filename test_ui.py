...
def test_analytics_page_shows_market_multiples_block(browser, base_url):
    ...
    try:
        ...
        # Медианы скрыты (SHOW_MULTIPLE_MEDIANS), и страница говорит об этом
        # своими словами. Проверяем СМЫСЛ, а не буквальную фразу: 7 сентября
        # 2026 по разбору рецензента формулировка стала «это сделки, которые
...
def test_deal_plate_hides_asset_text_that_admits_there_is_no_name(page, base_url):
    """Ячейка «Предмет сделки» пуста, только когда имени нет НИ У КОГО.

    Замена инварианта, а не отмена (9 сентября 2026). До этого дня ячейку
    гасило правило новизны (Этап 9, П6-9): предмет, повторяющий заголовок, не
    показывался. Владелец нашёл дважды подряд, что это неверно — «это уже не
    первый раз, когда предмет не показан в структуре, это почему вообще?»: в
    схеме из трёх клеток повтор заголовка не шум, а подпись к стрелке, и
    пустая середина читается как «мы не знаем, что купили». Замер по 1362
    покупкам и торгам: правило новизны называло предмет у 1123, правило
    «предмет назван» — у 1200.

    Эта карточка — единственная из 1362, где ячейка остаётся пустой ЧЕСТНО:
    в `asset` дословно стоит «неназванная брокерская компания», то есть сам
    источник говорит, что имени нет. Проверка сохранена именно ради этого
    случая: сплошное «показывать всё, что есть» напечатало бы признание в
    ячейку как имя.
    """
...
def test_deal_plate_names_the_asset_even_when_it_repeats_the_headline(page, base_url):
    """Вторая половина той же замены: предмет, повторяющий заголовок, ВИДЕН.

    «Аэрофлот» купил «Директ Стар», и предмет в карточке назван теми же
    словами, что в заголовке, — раньше правило новизны считало это шумом и
    оставляло среднюю клетку схемы пустой. Ровно на такую карточку и
    жаловался владелец.
    """
    visit(page, base_url, "#/deal/gc9ea6b08")
    plate = page.locator(".deal-plate")
    assert plate.locator(".dp-asset").count() == 1, \
        "предмет назван в карточке — схема обязана его показать"
    assert "Директ Стар" in plate.locator(".dp-asset").inner_text()
