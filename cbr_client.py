...
- `data101fnew_xml` (форма 101, оборотная ведомость) отдаёт реальные остатки
  ...
  знаков (30102 А, 30109 П — см. Положение 579-П, Раздел 3), а этот метод
  отдаёт уже свёрнутый до первого порядка остаток. Готовая, официально
  свёрнутая форма 0409806 («Всего активов», «Всего источников собственных
  средств») существует как отдельный метод (`GetF806Xml`/`GetF806Data`), но
  на практике отдаёт только шапку строк, ни одного числа — проверено на
  двух разных датах. Табличка `pipeline/cbr_account_types.py` — первый
  черновик классификации счетов первого порядка по данным Положения 579-П
  (Раздел 1 «Капитал», Раздел 3 «Межбанковские», см. `garant.ru`), НЕ
  довалидирована на полном наборе счетов реального банка — используйте
