...
# Этап 8, П3-8: «показывай ВСЕ показатели из БФО» — `bo()` уже отдаёт полный
# набор строк в `raw_lines` (сохраняется как есть в FinancialReport.raw_lines_json,
# см. pipeline/sync_fns.py), просто на экран шёл только выбор из 15 полей
# BO_LINES выше. Коды и дословные названия строк — коды строк ДЕЙСТВУЮЩЕЙ
# редакции форм 0710001/0710002/0710004 (Приказ Минфина №66н с изменениями
# 61н от 2019 года), сверены по независимой справочной таблице (Cbonds,
# «Список кодов показателей РСБУ») и живым ответом `bo()` — коды, реально
# встречающиеся в данных (Яндекс, ТехноНИКОЛЬ, Роснефть, Медскан), входят в
# эту таблицу без исключений. Приказ №157н (новая редакция форм) применяется
# «с отчётности за 2025 год» — то есть появится в ГИР БО не раньше 2026-го;
# на дату сверки (23-24 августа 2026) самый свежий год в живых ответах — 2024,
# редакция 66н/61н. Когда 2025-й год начнёт приходить от API, эту таблицу
# нужно будет перепроверить — не переносить вывод «коды не менялись» на
# данные, которых ещё не видели (родня урока про 809-П/579-П).
#
# Код 3600 «Чистые активы» относится к форме 0710003 (Отчёт об изменениях
# капитала) — единственная строка ЭТОЙ формы, которая встречается в `bo()`
# рядом с балансом/финрезультатами/ОДДС; отдельной секции под неё не
# заводим, показываем в конце раздела баланса как справочную строку.
FULL_LINE_SECTIONS: tuple[tuple[str, tuple[tuple[str, str], ...]], ...] = (
    ...
    ("IV. Долгосрочные обязательства", (
        ...
        ("1440", "Субсидии / долгосрочные производные финансовые инструменты"),
    ...
    ("Отчёт о финансовых результатах", (
        ...
        ("2130", "Расходы, связанные с разведкой и оценкой запасов нефти и газа"),
        ...
        ("2460", "Прочее"),
        ("2465", "Перераспределение налога на прибыль внутри консолидированной группы налогоплательщиков"),
        ("2400", "Чистая прибыль (убыток)"),
        ("2510", "Результат по переоценке внеоборотных активов, не включаемый в чистую прибыль (убыток)"),
        ("2520", "Результат от прочих операций, не включаемых в чистую прибыль (убыток) периода"),
    ...
    ("Денежные потоки от текущих операций", (
        ...
        ("4112", "от арендных платежей, лицензионных платежей, роялти, комиссионных и иных аналогичных платежей"),
        ("4113", "от перепродажи финансовых вложений"),
        ("4114", "от участия в других организациях и погашения векселей покупателей"),
        ("4115", "от реорганизации и продажи иного имущества"),
        ("4116", "от возмещения убытков и продажи валюты"),
        ("4117", "от возврата налогов и безвозмездных поступлений"),
        ("4118", "от погашения выданных беспроцентных займов"),
        ("4119", "прочие поступления"),
        ("4120", "Платежи"),
        ("4121", "поставщикам (подрядчикам) за сырьё, материалы, работы, услуги"),
        ("4122", "в связи с оплатой труда работников"),
        ("4123", "процентов по долговым обязательствам"),
        ("4124", "налога на прибыль организаций"),
        ("4125", "по иным налогам и сборам"),
        ("4126", "в государственные внебюджетные фонды"),
        ("4127", "по вознаграждениям членам совета директоров"),
        ("4128", "в связи с выдачей беспроцентных займов и благотворительностью"),
        ("4129", "прочие платежи"),
    ...
    ("Денежные потоки от инвестиционных операций", (
        ...
        ("4214", "дивидендов, процентов по долговым финансовым вложениям и аналогичных поступлений "
                 "от долевого участия в других организациях"),
        ("4215", "от возврата средств с банковских депозитов"),
        ("4216", "от погашения инвестиционных паёв"),
        ("4217", "от участия в совместной деятельности"),
        ("4218", "по производным финансовым инструментам"),
        ("4219", "прочие поступления"),
        ("4220", "Платежи"),
        ("4221", "в связи с приобретением, созданием, модернизацией, реконструкцией и подготовкой "
                 "к использованию внеоборотных активов"),
        ("4222", "в связи с приобретением акций других организаций (долей участия)"),
        ("4223", "в связи с приобретением долговых ценных бумаг (прав требования денежных средств "
        ...
        ("4225", "в связи с размещением средств в банковские депозиты"),
        ("4226", "в связи с вложением средств в имущество других организаций"),
        ("4227", "по производным финансовым инструментам"),
        ("4228", "в связи с приобретением инвестиционных паёв"),
        ("4229", "прочие платежи"),
    ...
    ("Денежные потоки от финансовых операций", (
        ...
        ("4315", "от бюджетных ассигнований"),
        ("4316", "от иного целевого финансирования"),
        ("4317", "от эмиссии опционов эмитента"),
        ("4318", "по договорам финансовой аренды"),
        ("4319", "прочие поступления"),
        ("4320", "Платежи"),
        ("4321", "собственникам (участникам) в связи с выкупом у них акций (долей участия) организации "
                 "или их выходом из состава участников"),
        ("4322", "на уплату дивидендов и иных платежей по распределению"),
        ("4323", "в связи с погашением (выкупом) векселей и других долговых ценных бумаг, возврат "
                 "кредитов и займов"),
        ("4324", "по договору финансовой аренды"),
        ("4325", "акционерам/учредителям при распределении между ними добавочного капитала"),
        ("4326", "выплаты участникам"),
        ("4327", "в связи с целевым финансированием"),
        ("4328", "в связи с эмиссией опционов эмитента"),
        ("4329", "прочие платежи"),
        ("4300", "Сальдо денежных потоков от финансовых операций"),
    )),
    ("Итог по движению денежных средств", (
        ("4400", "Сальдо денежных потоков за отчётный период"),
        ("4450", "Остаток денежных средств и денежных эквивалентов на начало отчётного периода"),
        ("4490", "Величина влияния изменений курса иностранной валюты по отношению к рублю"),
        ("4500", "Остаток денежных средств и денежных эквивалентов на конец отчётного периода"),
    )),
)
...
def full_lines_payload(raw_lines: dict) -> list[dict[str, Any]]:
    ...
    подменять отсутствующие данные правдоподобными», пустая строка честнее
    прочерка. Секция без единой заполненной строки тоже не попадает —
    родня уже применённому правилу для банковского баланса (`cbr_f806.py`)."""
